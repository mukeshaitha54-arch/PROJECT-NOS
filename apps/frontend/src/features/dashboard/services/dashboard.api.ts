...
export const dashboardApi = {
  ...
  async getOverview(): Promise<DashboardOverviewResponse> {
    ...
    >("/dashboard/overview");
    if (!res.data)
      throw new Error("Failed to retrieve dashboard overview counts.");
    return res.data;
  },
  ...
  async getDevices(
    ...
    const res = await apiClient.get<
      any,
      ApiResponse<PaginatedDashboardDevicesResponse>
    >(`/dashboard/devices?${params.toString()}`);
  ...
  async getDeviceDetail(id: string): Promise<DashboardDeviceDetailResponse> {
    const res = await apiClient.get<
      any,
      ApiResponse<DashboardDeviceDetailResponse>
    >(`/dashboard/device/${id}`);
    if (!res.data)
      throw new Error(
        "Failed to retrieve target device operational detail profile.",
      );
    return res.data;
  },
  ...
  async getDeviceHistory(
    ...
    const res = await apiClient.get<
      any,
      ApiResponse<PaginatedTelemetryResponse>
    >(`/dashboard/history/${deviceId}?${params.toString()}`);
    if (!res.data)
      throw new Error("Failed to retrieve historical telemetry records.");
    return res.data;
  },
  ...
  async getDeviceTimeline(
    ...
    const res = await apiClient.get<any, ApiResponse<any>>(
      `/device/${deviceId}/timeline?page=${page}&limit=${limit}`,
    );
    if (!res.data)
      throw new Error("Failed to retrieve device operational timeline.");
    return res.data;
  },
};
