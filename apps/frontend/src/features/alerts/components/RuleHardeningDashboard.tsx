...
export function RuleHardeningDashboard({
  selectedRuleId,
  isAdmin = true,
}: RuleHardeningDashboardProps) {
  ...
  const [validation, setValidation] = useState<RuleValidationResultDto | null>(
    null,
  );
  const [diff, setDiff] = useState<RuleDiffDto | null>(null);
  const [rollback, setRollback] = useState<RollbackPreviewDto | null>(null);
  const [metrics, setMetrics] = useState<RulePerformanceMetricsDto | null>(
    null,
  );
    ...
    >
      ...
      >
        <div>
          ...
          >
            Phase 5 Hardening — Zero ORM Leakage · Zero Polling · Fallback
            Resilient
          </p>
        </div>
        <button
          onClick={() => {
            fetchHealthAndQueues();
            if (selectedRuleId) fetchRuleTelemetry(selectedRuleId);
          }}
          disabled={loading}
          style={{
            display: "inline-flex",
      ...
      >
        {[
          {
            id: "HEALTH_QUEUES",
            label: "🛡️ Health & Queues",
            count: queues ? queues.totalFailed : 0,
          },
          {
          ...
          {
            id: "VALIDATION",
            label: "✔ Validation & Conflict Check",
            disabled: !selectedRuleId,
          },
          {
            id: "DIFF_ROLLBACK",
            label: "⟲ Version Diffs & Rollback",
            disabled: !selectedRuleId,
          },
          {
            id: "METRIC_STATS",
            label: "⚡ Performance & Usage Stats",
            disabled: !selectedRuleId,
          },
        ].map((tab) => (
          <button
            key={tab.id}
            onClick={() =>
              !tab.disabled && setActiveTab(tab.id as DashboardTab)
            }
        ...
        {activeTab === "SIMULATION_PREVIEW" && (
          ...
          >
            ...
            {preview ? (
              ...
              >
                Select rule parameters to preview impact...
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === "VALIDATION" && (
          <div>
            {validation ? (
              ...
              >
                Evaluating rule against dependency graph & conflicts...
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === "DIFF_ROLLBACK" && (
          ...
          >
            {diff ? (
              ...
              >
                No version history diffs loaded...
              </div>
            )}
            {rollback && (
              <RollbackPreviewPanel
                ...
                onConfirm={() => {
                  alert(
                    "Rollback command dispatched safely to enterprise audit log.",
                  );
                }}
