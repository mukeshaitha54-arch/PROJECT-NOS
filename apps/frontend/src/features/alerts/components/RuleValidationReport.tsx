  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        <div>
          <div style={{ fontSize: '14px', fontWeight: 700, color: result.valid ? '#10b981' : '#ef4444' }}>
              ...
              <div key={i} style={{
                ...
                <div>
                  <div style={{ fontSize: '12px', color: '#fca5a5', fontWeight: 600 }}>
                    [{err.code}] {err.field && <span style={{ color: '#64748b' }}>({err.field})</span>}
                  </div>
                  <div style={{ fontSize: '11px', color: '#94a3b8', marginTop: '2px' }}>{err.message}</div>
                  {err.conflictingRuleName && (
                    <div style={{ fontSize: '10px', color: '#64748b', marginTop: '2px' }}>
                      Conflicts with: <span style={{ color: '#94a3b8' }}>{err.conflictingRuleName}</span>
                    </div>
                  )}
                </div>
              </div>
      ...
      {result.warnings.length > 0 && (
        <div>
          <div style={{ fontSize: '11px', fontWeight: 700, color: '#f59e0b', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '6px' }}>
            Warnings
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
            {result.warnings.map((warn, i) => (
              <div key={i} style={{
                ...
                <div>
                  <div style={{ fontSize: '12px', color: '#fcd34d', fontWeight: 600 }}>[{warn.code}]</div>
                  <div style={{ fontSize: '11px', color: '#94a3b8', marginTop: '2px' }}>{warn.message}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {result.circularDependencies && result.circularDependencies.length > 0 && (
        <div style={{
          padding: '10px 12px',
          background: 'rgba(239, 68, 68, 0.06)',
          border: '1px solid rgba(239, 68, 68, 0.2)',
      ...
      {result.duplicateOf && (
        <div style={{
          ...
          ⚠ This rule duplicates existing rule ID: <span style={{ fontFamily: 'monospace' }}>{result.duplicateOf}</span>
        </div>
      )}
    </div>
  );
