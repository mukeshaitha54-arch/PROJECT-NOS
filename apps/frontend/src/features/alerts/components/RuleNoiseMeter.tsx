...
function getRating(score: number): { label: string; color: string; bg: string } {
  if (score <= 25) return { label: 'LOW', color: '#10b981', bg: 'rgba(16,185,129,0.15)' };
  if (score <= 50) return { label: 'MEDIUM', color: '#f59e0b', bg: 'rgba(245,158,11,0.15)' };
  if (score <= 75) return { label: 'HIGH', color: '#f97316', bg: 'rgba(249,115,22,0.15)' };
...
export function RuleNoiseMeter({ score, showBreakdown = false, breakdown, recommendation }: RuleNoiseMeterProps) {
        ...
        <div style={{
          ...
          {Object.entries(breakdown).map(([key, val]) => (
            <div key={key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '11px', color: '#64748b' }}>{BREAKDOWN_LABELS[key] || key}</span>
              <span style={{ fontSize: '11px', color: '#94a3b8', fontWeight: 600, fontFamily: 'monospace' }}>
                {val}pts
              </span>
            </div>
          ))}
