...
export function RuleHealthBadge({
  status,
  redisConnected = true,
  compact = false,
}: RuleHealthBadgeProps) {
...
export function RuleHealthSummaryCard({ health }: RuleHealthSummaryProps) {
  const metrics = [
    ...
    {
      label: "Conflicting",
      value: health.conflictingRules,
      color: health.conflictingRules > 0 ? "#f59e0b" : "#64748b",
    },
    {
      label: "Duplicates",
      value: health.duplicateRules,
      color: health.duplicateRules > 0 ? "#ef4444" : "#64748b",
    },
    {
      label: "Avg Eval",
      value: `${health.avgEvaluationMs.toFixed(1)}ms`,
      color: health.avgEvaluationMs > 300 ? "#f97316" : "#10b981",
    },
  ];
    ...
    >
      ...
      >
        ...
        <span style={{ fontSize: "12px", color: "#94a3b8" }}>
          ...
          {health.redis?.connected ? (
            <>
              Connected · {health.redis.latencyMs}ms latency ·{" "}
              {(health.redis.memoryUsageBytes / 1024 / 1024).toFixed(1)}MB
            </>
          ) : (
      ...
      {health.slowRules?.length > 0 && (
        ...
        >
          ...
          >
            ⚠ {health.slowRules.length} Slow Rule
            {health.slowRules.length !== 1 ? "s" : ""} Detected
          </div>
          {health.slowRules.slice(0, 3).map((r) => (
            <div
              key={r.id}
              style={{ fontSize: "11px", color: "#94a3b8", paddingLeft: "8px" }}
            >
