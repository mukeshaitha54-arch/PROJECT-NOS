...
export function RollbackPreviewPanel({ preview, onConfirm, onCancel, isLoading }: RollbackPreviewPanelProps) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: '15px', fontWeight: 700, color: '#f97316', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span>⟲</span> Rollback Preview
          </div>
          <div style={{ fontSize: '12px', color: '#94a3b8', marginTop: '4px' }}>
            {preview.ruleName} · v{preview.currentVersion} → v{preview.targetVersion}
          </div>
        </div>
        <div style={{
          padding: '4px 12px',
      ...
      <div style={{
        ...
        {preview.isRollbackSafe ? '✓ Rollback is safe — minimal changes detected' : '⚠ Rollback may have significant impact — review carefully'}
      </div>
      ...
      {preview.differences.length > 0 && (
        <div>
          <div style={{ fontSize: '11px', fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '8px' }}>
            {preview.differences.length} field{preview.differences.length !== 1 ? 's' : ''} will change
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {preview.differences.map(diff => (
              <div key={diff.field} style={{
                ...
                <span style={{ fontSize: '11px', color: '#64748b' }}>{diff.field}</span>
                <span style={{
                  fontSize: '12px',
        ...
        {onConfirm && (
          <button
            ...
            style={{
              padding: '8px 18px',
              borderRadius: '8px',
              background: isLoading ? 'rgba(249,115,22,0.3)' : 'rgba(249, 115, 22, 0.85)',
