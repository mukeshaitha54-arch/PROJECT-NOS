        ...
        {[
          {
            label: "Wait",
            value: queue.waiting,
            color: queue.waiting > 50 ? "#f59e0b" : "#94a3b8",
          },
          {
            label: "Active",
            value: queue.active,
            color: queue.active > 0 ? "#22d3ee" : "#94a3b8",
          },
          { label: "Done", value: queue.completed, color: "#10b981" },
          {
            label: "Failed",
            value: queue.failed,
            color: queue.failed > 0 ? "#ef4444" : "#94a3b8",
          },
...
export function RuleUsagePanel({
  dashboard,
  onPurge,
  canPurge = false,
}: RuleUsagePanelProps) {
      ...
      >
        <div>
          ...
          <div style={{ fontSize: "11px", color: "#64748b", marginTop: "2px" }}>
            {dashboard.totalWaiting} waiting · {dashboard.totalActive} active ·{" "}
            {dashboard.totalFailed} failed
          </div>
        </div>
        <span
          style={{
        ...
        {dashboard.redis.connected && (
          ...
          >
            {[
              {
                label: "Memory",
                value: `${(dashboard.redis.memoryUsageBytes / 1024 / 1024).toFixed(1)}MB`,
              },
              { label: "Latency", value: `${dashboard.redis.latencyMs}ms` },
              {
                label: "Clients",
                value: String(dashboard.redis.connectedClients),
              },
              {
                label: "Uptime",
                value: `${Math.round(dashboard.redis.uptimeSeconds / 3600)}h`,
              },
            ].map((m) => (
              <div key={m.label} style={{ textAlign: "center" }}>
                <div
