...
export const alertApi = {
  ...
  async getAlerts(query: AlertQueryParams = {}) {
    ...
    if (query.snoozed !== undefined)
      params.append("snoozed", String(query.snoozed));
  ...
  async getStatistics(): Promise<AlertStatisticsDto> {
    const res = await apiClient.get<any, ApiResponse<AlertStatisticsDto>>(
      "/alerts/statistics",
    );
    return res.data!;
  },
  ...
  async updateStatus(id: string, status: string, comment = "") {
    const res = await apiClient.patch<any, ApiResponse<any>>(
      `/alerts/${id}/status`,
      { status, comment },
    );
    return res.data!;
  ...
  async bulkOperation(
    ...
    const res = await apiClient.post<any, ApiResponse<any>>("/alerts/bulk", {
      ids,
      action,
    });
    return res.data!;
  ...
  async getRules(): Promise<AlertRuleEnhancedDto[]> {
    const res = await apiClient.get<any, ApiResponse<AlertRuleEnhancedDto[]>>(
      "/alerts/rules",
    );
    return res.data || [];
  },

  async searchRules(params: RuleSearchParams): Promise<{
    data: AlertRuleEnhancedDto[];
    ...
    if (params.enabled !== undefined)
      qs.append("enabled", String(params.enabled));
    ...
    const res = await apiClient.get<any>(
      `/alerts/rules/search?${qs.toString()}`,
    );
    return res.data;
  },
  ...
  async createRule(data: Partial<AlertRuleEnhancedDto>) {
    const res = await apiClient.post<any, ApiResponse<AlertRuleEnhancedDto>>(
      "/alerts/rules",
      data,
    );
    return res.data!;
  ...
  async updateRule(id: string, data: Partial<AlertRuleEnhancedDto>) {
    const res = await apiClient.put<any, ApiResponse<AlertRuleEnhancedDto>>(
      `/alerts/rules/${id}`,
      data,
    );
    return res.data!;
  ...
  async deleteRule(id: string) {
    const res = await apiClient.delete<any, ApiResponse<{ success: boolean }>>(
      `/alerts/rules/${id}`,
    );
    return res.data!;
  },
  ...
  async rollbackRule(
    ...
    version: number,
    reason?: string,
  ): Promise<AlertRuleEnhancedDto> {
    const res = await apiClient.post<any, ApiResponse<AlertRuleEnhancedDto>>(
      `/alerts/rules/${id}/rollback/${version}`,
      { reason },
    );
    return res.data!;
  },
  ...
  async getCategories(): Promise<string[]> {
    const res = await apiClient.get<any, ApiResponse<string[]>>(
      "/alerts/rules/categories",
    );
    return res.data || [];
  },
  ...
  async exportRules(): Promise<RuleExportDto> {
    const res = await apiClient.get<any, ApiResponse<RuleExportDto>>(
      "/alerts/rules/export",
    );
    return res.data!;
  },
  ...
  async importRules(exportData: RuleExportDto): Promise<RuleImportResultDto> {
    const res = await apiClient.post<any, ApiResponse<RuleImportResultDto>>(
      "/alerts/rules/import",
      exportData,
    );
    return res.data!;
  ...
  async getDependencyGraph(): Promise<RuleDependencyGraphDto> {
    const res = await apiClient.get<any, ApiResponse<RuleDependencyGraphDto>>(
      "/alerts/rules/dependencies",
    );
    return res.data!;
  },
  ...
  async testRule(
    ...
    const res = await apiClient.post<any, ApiResponse<RuleTestResultDto>>(
  ...
  async validateRule(
    data: Partial<AlertRuleEnhancedDto>,
    excludeId?: string,
  ): Promise<RuleValidationResultDto> {
    const url = excludeId
      ? `/alerts/rules/validate?excludeId=${excludeId}`
      : "/alerts/rules/validate";
    const res = await apiClient.post<any, ApiResponse<RuleValidationResultDto>>(
      url,
      data,
    );
    return res.data!;
  ...
  async previewRule(
    ...
    const res = await apiClient.post<any, ApiResponse<RulePreviewDto>>(
      "/alerts/rules/preview",
      data,
    );
    return res.data!;
  },
  ...
  async dryRun(data: Partial<AlertRuleEnhancedDto>): Promise<DryRunResultDto> {
    const res = await apiClient.post<any, ApiResponse<DryRunResultDto>>(
      "/alerts/rules/dry-run",
      data,
    );
    return res.data!;
  },
  ...
  async getRuleDiff(
    ...
    if (toVersion !== undefined)
      url += `${fromVersion !== undefined ? "&" : "?"}toVersion=${toVersion}`;
  ...
  async getRollbackPreview(
    ...
    const res = await apiClient.get<any, ApiResponse<RollbackPreviewDto>>(
      `/alerts/rules/${id}/rollback-preview/${version}`,
    );
    return res.data!;
  },
  ...
  async replayHistoricalTelemetry(
    ...
    const res = await apiClient.post<any, ApiResponse<ReplayResultDto>>(
      `/alerts/rules/${ruleId}/replay`,
      { from, to, deviceIds },
    );
    return res.data!;
  },
  ...
  async getRulesHealth(): Promise<RuleHealthDto> {
    const res = await apiClient.get<any, ApiResponse<RuleHealthDto>>(
      "/alerts/rules/health",
    );
    return res.data!;
  },
  ...
  async getRuleMetrics(id: string): Promise<RulePerformanceMetricsDto> {
    const res = await apiClient.get<
      any,
      ApiResponse<RulePerformanceMetricsDto>
    >(`/alerts/rules/${id}/metrics`);
    return res.data!;
  },
  ...
  async getRuleComplexity(id: string): Promise<RuleComplexityBreakdownDto> {
    const res = await apiClient.get<
      any,
      ApiResponse<RuleComplexityBreakdownDto>
    >(`/alerts/rules/${id}/complexity`);
    return res.data!;
  },

  async getRuleNoiseScore(id: string): Promise<RuleNoiseScoreDto> {
    const res = await apiClient.get<any, ApiResponse<RuleNoiseScoreDto>>(
      `/alerts/rules/${id}/noise`,
    );
    return res.data!;
  },

  async getRuleUsageStatistics(id: string): Promise<RuleUsageStatisticsDto> {
    const res = await apiClient.get<any, ApiResponse<RuleUsageStatisticsDto>>(
      `/alerts/rules/${id}/usage`,
    );
    return res.data!;
  },
  ...
  async getRuleAuditTrail(
    ...
    skip = 0,
    take = 50,
  ): Promise<{ entries: RuleAuditEntryDto[]; total: number }> {
    const res = await apiClient.get<
      any,
      ApiResponse<{ entries: RuleAuditEntryDto[]; total: number }>
    >(`/alerts/rules/${id}/audit?skip=${skip}&take=${take}`);
    return res.data!;
  },

  async getRecommendations(): Promise<RuleRecommendationDto[]> {
    const res = await apiClient.get<any, ApiResponse<RuleRecommendationDto[]>>(
      "/alerts/rules/recommendations",
    );
    return res.data || [];
  },
  ...
  async getDlqLogs() {
    const res = await apiClient.get<
      any,
      ApiResponse<{ logs: any[]; total: number }>
    >("/alerts/notifications/dlq");
    return res.data!;
  },

  async retryDlq(logId: string) {
    const res = await apiClient.post<any, ApiResponse<{ retried: boolean }>>(
      `/alerts/notifications/dlq/${logId}/retry`,
      {},
    );
    return res.data!;
  },
  ...
  async getQueueDashboard(): Promise<QueueDashboardDto> {
    const res = await apiClient.get<any, ApiResponse<QueueDashboardDto>>(
  ...
  async retryQueueJob(
    ...
    const res = await apiClient.post<any, ApiResponse<{ retried: boolean }>>(
      `/alerts/queue/${queueName}/retry/${jobId}`,
      {},
    );
    return res.data!;
  },
  ...
  async purgeQueue(queueName: string): Promise<{ purged: number }> {
    const res = await apiClient.delete<any, ApiResponse<{ purged: number }>>(
      `/alerts/queue/${queueName}/purge`,
    );
    return res.data!;
  },
  ...
  async getMaintenanceWindows(deviceId?: string) {
    const url = deviceId
      ? `/maintenance-windows?deviceId=${deviceId}`
      : "/maintenance-windows";
  ...
  async simulateRule(
    ...
    const res = await apiClient.post<any, ApiResponse<any>>(
      "/alerts/simulate",
      { metric, operator, threshold, timeRangeHours },
    );
    return res.data!;
  },
};
