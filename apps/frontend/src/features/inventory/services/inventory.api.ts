...
export const inventoryApi = {
  ...
  async getCompleteInventory(
    ...
    >(`/inventory/${deviceId}`);
    if (!res.data)
      throw new Error("Failed to retrieve complete device inventory.");
    return res.data;
  },
  ...
  async getHardware(deviceId: string): Promise<HardwareInventoryResponse> {
    const res = await apiClient.get<
      any,
      ApiResponse<HardwareInventoryResponse>
    >(`/inventory/hardware/${deviceId}`);
    if (!res.data)
      throw new Error("Failed to retrieve hardware inventory specification.");
    return res.data;
  },
  ...
  async getSoftware(
    ...
    if (!res.data)
      throw new Error("Failed to retrieve software and services inventory.");
    return res.data;
  },
  ...
  async getNetwork(deviceId: string): Promise<NetworkInventoryResponse> {
    const res = await apiClient.get<any, ApiResponse<NetworkInventoryResponse>>(
      `/inventory/network/${deviceId}`,
    );
    if (!res.data)
      throw new Error("Failed to retrieve network adapter inventory.");
    return res.data;
  },
  ...
  async getSecurity(deviceId: string): Promise<SecurityInventoryResponse> {
    const res = await apiClient.get<
      any,
      ApiResponse<SecurityInventoryResponse>
    >(`/inventory/security/${deviceId}`);
    if (!res.data)
      throw new Error(
        "Failed to retrieve security and system capability inventory.",
      );
    return res.data;
  },
  ...
  async getHealth(): Promise<InventoryHealthResponse> {
    const res = await apiClient.get<any, ApiResponse<InventoryHealthResponse>>(
      "/inventory/health",
    );
    if (!res.data)
      throw new Error(
        "Failed to retrieve enterprise inventory health diagnostics.",
      );
    return res.data;
  },
  ...
  async triggerScan(
    ...
    if (!res.data)
      throw new Error("Failed to schedule manual inventory re-scan.");
    return res.data;
  },
};
