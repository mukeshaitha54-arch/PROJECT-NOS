  ...
  useEffect(() => {
    ...
    unsubscribers.push(
      subscribe(SocketEvents.TELEMETRY_RECEIVED, (env) => {
        if (
          env.payload?.deviceId === deviceId ||
          (env.payload as any)?.id === deviceId
        ) {
    ...
    unsubscribers.push(
      subscribe(SocketEvents.INVENTORY_UPDATED, (env) => {
        if (env.payload?.deviceId === deviceId) {
          handlersRef.current.onInventory?.(
            env.payload as RealtimeInventoryEvent,
            env,
          );
        }
      }),
      ...
      subscribe(SocketEvents.DEVICE_OFFLINE, (env) => {
        if (env.payload?.deviceId === deviceId) {
          handlersRef.current.onStatusChange?.(
            "OFFLINE",
            env.payload?.reason || "DISCONNECTED",
            env,
          );
        }
      }),
