...
export const RealtimeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = realtimeClient.subscribeStatus((newStatus, newLatency) => {
      setStatus(newStatus);
  ...
  const subscribe = useCallback(<T = any>(event: SocketEvents | string, callback: (envelope: SocketEventEnvelope<T>) => void) => {
