...
export const deviceApi = {
  ...
  async getStatus(): Promise<DeviceStatusResponse> {
    const res = await apiClient.get<any, ApiResponse<DeviceStatusResponse>>('/device/status');
    if (!res.data) throw new Error('Failed to parse device status response payload.');
    return res.data;
  },
  ...
  async getById(id: string): Promise<Device & { lastHeartbeat?: Heartbeat | null }> {
    const res = await apiClient.get<any, ApiResponse<Device & { lastHeartbeat?: Heartbeat | null }>>(`/device/${id}`);
    if (!res.data) throw new Error('Failed to retrieve target device identity profile.');
    return res.data;
  },
  ...
  async register(payload: RegisterDevicePayload): Promise<RegisterDeviceResponse> {
    const res = await apiClient.post<any, ApiResponse<RegisterDeviceResponse>>('/device/register', payload);
    if (!res.data) throw new Error('Failed to complete agent onboarding registration.');
    return res.data;
  },
  ...
  async sendHeartbeat(payload: HeartbeatPayload, token: string): Promise<HeartbeatResponse> {
    const res = await apiClient.post<any, ApiResponse<HeartbeatResponse>>('/device/heartbeat', payload, {
      headers: { 'X-Device-Token': token },
    });
    if (!res.data) throw new Error('Heartbeat transmission rejected by server.');
    return res.data;
  },
  ...
  async getLatestTelemetry(deviceId: string): Promise<TelemetrySnapshot | null> {
    try {
      const res = await apiClient.get<any, ApiResponse<TelemetrySnapshot>>(`/telemetry/latest/${deviceId}`);
  ...
  async getTelemetryHistory(deviceId: string, query?: TelemetryHistoryQuery): Promise<PaginatedTelemetryResponse> {
    ...
    const res = await apiClient.get<ApiResponse<any> & PaginatedTelemetryResponse>(`/telemetry/history/${deviceId}?${params.toString()}`);
    return {
