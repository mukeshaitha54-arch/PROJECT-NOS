...
export const useGlobalStore = create<GlobalStoreState>((set) => ({
  ...
  updateDevicePresence: (deviceId, status, lastSeen) =>
    set((state) => ({
      devices: state.devices.map((device) =>
        device.id === deviceId ? { ...device, status, lastSeen } : device,
      ),
    })),

  appendTelemetry: (deviceId, data) =>
    set((state) => {
  ...
  prependAlert: (alert) =>
    set((state) => {
      const newAlerts = [
        alert,
        ...state.alerts.filter((a) => a.id !== alert.id),
      ];
      // Keep top 10 recent
      newAlerts.sort(
        (a, b) =>
