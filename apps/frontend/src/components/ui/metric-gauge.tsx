      ...
      <div className="relative" style={{ width: cfg.size, height: cfg.size }}>
        ...
        {showValue && (
          <div className="absolute inset-0 flex items-center justify-center">
            <span className={`font-bold tabular-nums ${cfg.fontSize} ${textColorClass}`}>
              {clamped.toFixed(0)}%
            </span>
          </div>
        )}
...
export function MetricBar({
  ...
  const clamped = Math.max(0, Math.min(100, value ?? 0));
  const barColor =
    variant === 'network' ? 'bg-cyan-500' :
    clamped >= 90 ? 'bg-red-500' :
    clamped >= 75 ? 'bg-amber-500' :
    clamped >= 60 ? 'bg-yellow-400' :
    'bg-emerald-500';
    ...
    <div className="space-y-1">
      {label && (
        <div className="flex justify-between items-center">
          ...
          <span className="text-xs font-mono font-semibold text-slate-300">{clamped.toFixed(1)}%</span>
        </div>
      )}
      <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden" aria-label={`${label}: ${clamped}%`}>
        <div
