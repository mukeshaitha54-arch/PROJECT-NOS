...
export const useAlerts = ({
  ...
  >({
    ...
    queryFn: () =>
      api.get<PaginatedResponse<Alert>>(
        `/alerts/active?${queryParams.toString()}`,
      ),
    staleTime: 5000,
  ...
  const handleOptimisticUpdate = (id: string, newStatus: Alert["status"]) => {
    ...
    const previousAlerts = queryClient.getQueryData<PaginatedResponse<Alert>>([
      "alerts",
      page,
      limit,
      status,
    ...
    if (previousAlerts) {
      queryClient.setQueryData<PaginatedResponse<Alert>>(
        ["alerts", page, limit, status, severity],
        {
          ...previousAlerts,
  ...
  const handleError = (
    err: unknown,
    variables: string,
    context?: { previousAlerts?: PaginatedResponse<Alert> },
  ) => {
