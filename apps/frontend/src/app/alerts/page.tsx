...
import {
  alertApi,
  AlertQueryParams,
} from "../../features/alerts/services/alert-api.service";
import {
  AlertStatisticsDto,
  AlertSeverity,
  AlertStatus,
...
function EnterpriseAlertsCommandCenterPageContent() {
  const [activeTab, setActiveTab] = useState<"INCIDENTS" | "SIMULATOR" | "DLQ">(
    "INCIDENTS",
  );
  ...
  const loadIncidentsAndStats = useCallback(async () => {
    try {
      ...
      setError(
        err.message ||
          "Failed to retrieve operational incident telemetry from control plane.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const runSimulator = async (e: React.FormEvent) => {
    ...
    try {
      setSimLoading(true);
      const res = await alertApi.simulateRule(
        simMetric,
        simOperator,
        simThreshold,
        24,
  ...
  const getSeverityBadge = (sev: string) => {
    switch (sev) {
      case "CRITICAL":
        return (
          <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold bg-rose-950/80 text-rose-300 border border-rose-600 shadow-sm shadow-rose-900/50 animate-pulse">
            <Flame className="w-3 h-3 mr-1 text-rose-400" /> CRITICAL
          </span>
        );
      case "HIGH":
        return (
          <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold bg-amber-950/80 text-amber-300 border border-amber-600">
            <AlertTriangle className="w-3 h-3 mr-1 text-amber-400" /> HIGH
          </span>
        );
      case "MEDIUM":
        return (
          <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold bg-yellow-950/80 text-yellow-300 border border-yellow-600/60">
            <Activity className="w-3 h-3 mr-1 text-yellow-400" /> MEDIUM
          </span>
        );
      case "LOW":
        return (
          <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold bg-blue-950/80 text-blue-300 border border-blue-600/60">
            LOW
          </span>
        );
      default:
  ...
  const getStatusBadge = (st: string) => {
    switch (st) {
      ...
      case "OPEN":
        return (
          <span className="px-2 py-0.5 rounded text-xs font-mono font-semibold bg-sky-900/70 text-sky-200 border border-sky-600/60">
            OPEN
          </span>
        );
      case "ACKNOWLEDGED":
        return (
          <span className="px-2 py-0.5 rounded text-xs font-mono font-semibold bg-emerald-900/70 text-emerald-200 border border-emerald-600/60">
            ACKNOWLEDGED
          </span>
        );
      case "SNOOZED":
      case "SUPPRESSED":
        return (
          <span className="px-2 py-0.5 rounded text-xs font-mono font-semibold bg-indigo-950 text-indigo-300 border border-indigo-700">
            SNOOZED
          </span>
        );
      case "RESOLVED":
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100 p-6 md:p-8 space-y-8 font-sans selection:bg-rose-500/30">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-6 border-b border-slate-800/80">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs font-medium text-slate-400 mt-1 flex items-center gap-2">
                Phase 5 Alert & Notification Engine • O(1) SHA256 Fingerprint
                Deduplication • Multi-Channel DLQ & Automated SLA Escalations
              </p>
            </div>
          </div>
        </div>
        <div className="flex items-center space-x-4 w-full md:w-auto justify-end">
          ...
          >
            <RefreshCw
              className={`w-5 h-5 ${loading ? "animate-spin text-rose-400" : ""}`}
            />
          </button>
        </div>
      </div>
          ...
          {statistics && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <div className="p-5 rounded-2xl bg-rose-950/40 border border-rose-800/50 shadow-xl backdrop-blur-sm relative overflow-hidden group hover:border-rose-700 transition-all">
                ...
                <p className="mt-2 text-3xl font-extrabold text-rose-300 tracking-tight">
              ...
              <div className="p-5 rounded-2xl bg-amber-950/30 border border-amber-800/40 shadow-xl backdrop-blur-sm relative overflow-hidden group hover:border-amber-700 transition-all">
                ...
                <p className="mt-2 text-3xl font-extrabold text-amber-300 tracking-tight">
              ...
              <div className="p-5 rounded-2xl bg-indigo-950/30 border border-indigo-800/40 shadow-xl backdrop-blur-sm relative overflow-hidden group hover:border-indigo-700 transition-all">
                ...
                <p className="mt-2 text-3xl font-extrabold text-indigo-300 tracking-tight">
                  {statistics.byStatus?.SNOOZED ||
                    0 + (statistics.byStatus?.SUPPRESSED || 0)}
                </p>
              ...
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <div>
                  <h2 className="text-base font-bold text-white flex items-center gap-2">
                    ...
                    <span>
                      Live Incident Aging Breakdown & SLA Matrix (Zero-Chart
                      Analytics)
                    </span>
                  </h2>
                  <p className="text-xs text-slate-400">
                    Real-time unaddressed duration buckets across enterprise
                    operations fleet.
                  </p>
                </div>
                ...
                <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 hover:border-slate-700 transition-all">
                  ...
                  <div className="flex items-baseline justify-between mt-1.5">
                    <span className="text-2xl font-black text-emerald-400 font-mono">
                      {statistics.agingBuckets.bucket0to5m}
                    </span>
                ...
                <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 hover:border-slate-700 transition-all">
                  ...
                  <div className="flex items-baseline justify-between mt-1.5">
                    <span className="text-2xl font-black text-yellow-400 font-mono">
                      {statistics.agingBuckets.bucket5to15m}
                    </span>
                ...
                <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 hover:border-slate-700 transition-all">
                  ...
                  <div className="flex items-baseline justify-between mt-1.5">
                    <span className="text-2xl font-black text-amber-400 font-mono">
                      {statistics.agingBuckets.bucket15to60m}
                    </span>
                ...
                <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 hover:border-slate-700 transition-all">
                  ...
                  <div className="flex items-baseline justify-between mt-1.5">
                    <span className="text-2xl font-black text-rose-400 font-mono">
                      {statistics.agingBuckets.bucket1to4h}
                    </span>
                ...
                <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 hover:border-slate-700 transition-all">
                  ...
                  <div className="flex items-baseline justify-between mt-1.5">
                    <span className="text-2xl font-black text-rose-500 font-mono">
                      {statistics.agingBuckets.bucket4to24h}
                    </span>
                ...
                <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 hover:border-slate-700 transition-all">
                  ...
                  <div className="flex items-baseline justify-between mt-1.5">
                    <span className="text-2xl font-black text-slate-400 font-mono">
                      {statistics.agingBuckets.bucket24hPlus}
                    </span>
                    <span className="text-[10px] text-slate-500 font-medium">
                      Stale / Snoozed
                    </span>
            ...
            <div className="flex flex-col xl:flex-row justify-between xl:items-center gap-4 p-4 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-lg">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-400 mr-2 flex items-center gap-1">
                  <Filter className="w-3.5 h-3.5 text-rose-400" /> Saved
                  Filters:
                </span>
                {[
                  ...
                  {
                    id: "UNACKNOWLEDGED",
                    label: "Unacknowledged (Needs Action)",
                  },
                ].map((pill) => (
                  <button
                    key={pill.id}
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <span className="text-sm font-extrabold text-white">
                      {selectedIds.length} Incident
                      {selectedIds.length > 1 ? "s" : ""} Selected for Bulk
                      Operation
                    </span>
                    <p className="text-xs text-rose-300">
                      Operations executed here are recorded directly to the
                ...
                <tbody className="divide-y divide-slate-800/60 text-xs">
                  {loading && alerts.length === 0 ? (
                    ...
                    <tr>
                      <td
                        colSpan={9}
                        className="py-16 text-center text-slate-500 font-semibold"
                      >
                        No operational alert incidents match your filter
                        criteria. Fleet is operating optimally.
                      </td>
                    </tr>
                  ) : (
                    alerts.map((a) => (
                      <tr
                        key={a.id}
                        className={`hover:bg-slate-800/50 transition-colors group ${selectedIds.includes(a.id) ? "bg-rose-950/20" : ""}`}
                      >
                        ...
                        <td className="py-3 px-4 font-mono font-extrabold text-white">
                          <Link
                            href={`/alerts/${a.id}`}
                            className="hover:text-rose-400 transition-colors inline-flex items-center gap-1 group/link"
                          >
                        ...
                        <td className="py-3 px-4">
                          ...
                          <span className="text-slate-400 text-[11px] font-mono mt-0.5 block line-clamp-1">
                            {a.description}
                          </span>
                        </td>
                        <td className="py-3 px-4 font-mono text-slate-300 font-semibold">
                          <Link
                            href={`/device/${a.deviceId}`}
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center space-x-1.5">
                            <span
                              ...
                              title={`SHA256: ${a.fingerprint || "N/A"}`}
                            >
                              {a.fingerprint
                                ? `fp: ${a.fingerprint.slice(0, 8)}...`
                                : "no-fp"}
                            </span>
                          ...
                          <div className="flex items-center space-x-2">
                            <div className="w-12 bg-slate-800 rounded-full h-1.5 overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${Math.min(100, a.riskScore || 20)}%`,
                                }}
                              ></div>
                            </div>
                          ...
                          <div className="space-y-1">
                            ...
                            <span className="block text-[10px] text-slate-400 font-medium">
                              ...
                              <span className="font-mono text-slate-300">
                                {a.assignedUserId || "Standby Operators"}
                              </span>
                            </span>
                          </div>
                        </td>
          ...
          <div className="lg:col-span-1 p-6 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-2xl space-y-6">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">
                Test custom threshold logic against the last 24 hours of
                historical operational telemetry before activating in
                production.
              </p>
            </div>
          ...
          <div className="lg:col-span-2 p-6 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-2xl space-y-6">
            <div className="border-b border-slate-800 pb-4">
              <h3 className="text-base font-extrabold text-white">
                Simulation Analysis & Cooldown Impact Report
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">
                Estimated production blast radius calculated over 14,400+
                historical telemetry points.
              </p>
            </div>
            ...
            {simReport ? (
              <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="p-5 rounded-xl bg-slate-950 border border-slate-800 text-center">
                    ...
                    <span className="text-3xl font-extrabold font-mono text-amber-400 mt-2 block">
                      {simReport.wouldTriggerCount}
                    </span>
                    <span className="text-[11px] text-slate-500 mt-1 block">
                      Gross anomaly events detected
                    </span>
                  </div>
                  <div className="p-5 rounded-xl bg-slate-950 border border-slate-800 text-center">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">
                  ...
                  <div className="p-5 rounded-xl bg-slate-950 border border-slate-800 text-center">
                    ...
                    <span className="text-3xl font-extrabold font-mono text-rose-400 mt-2 block">
                      {simReport.realAlertsCount}
                    </span>
                ...
                <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-3">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">
                    Affected Node Infrastructure (
                    {simReport.affectedDevices?.length || 0})
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    {simReport.affectedDevices?.map((dev: any) => (
                      <span
                        key={dev.deviceId || dev}
                        className="px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-xs font-mono font-bold text-sky-300"
                      >
              ...
              <div className="py-20 text-center text-slate-500">
                ...
                <p className="font-semibold text-sm">
                  Configure threshold metrics on the left and click execute to
                  view simulated production impacts.
                </p>
              </div>
            )}
          </div>
            ...
            <div className="flex justify-between items-center border-b border-slate-800 pb-4">
              <div>
                <h3 className="text-base font-extrabold text-white flex items-center gap-2">
                  ...
                  <span>
                    Dead Letter Queue (DLQ) & Notification Delivery Receipts
                  </span>
                </h3>
                <p className="text-xs text-slate-400 mt-0.5">
                  Inspect multi-channel email, Slack, and Discord dispatch logs
                  and manually re-attempt failed deliveries.
                </p>
              </div>
              <button
                onClick={loadDlqLogs}
              ...
              >
                <RefreshCw
                  className={`w-3.5 h-3.5 ${dlqLoading ? "animate-spin" : ""}`}
                />
                ...
                <tbody className="divide-y divide-slate-800 text-xs">
                  {dlqLogs.length === 0 ? (
                    <tr>
                      ...
                      >
                        Dead Letter Queue is clean! All enterprise multi-channel
                        notifications delivered successfully with 0 failures.
                      </td>
                    </tr>
                  ) : (
                    dlqLogs.map((log) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-4 font-extrabold text-sky-400 uppercase">
                          {log.provider}
                        </td>
                        ...
                        <td className="py-3 px-4">
                          {log.status === "SUCCESS" ? (
                            ...
                            <span className="text-rose-400 font-bold bg-rose-950/50 px-2 py-1 rounded">
                              FAILED (DLQ)
                            </span>
                          )}
                        </td>
                        ...
                        <td className="py-3 px-4 text-rose-300 font-mono text-xs">
...
const DynamicEnterpriseAlertsCommandCenterPage = nextDynamic(
  () => Promise.resolve(EnterpriseAlertsCommandCenterPageContent),
  { ssr: false },
);
