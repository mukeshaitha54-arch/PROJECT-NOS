...
import {
  ShieldAlert, AlertTriangle, CheckCircle, Clock, ChevronLeft,
...
export default function EnterpriseIncidentDetailWorkspacePage() {
  ...
  const loadIncidentProfile = useCallback(async () => {
    ...
    try {
      ...
      setHistory(res.history || [
        { id: 'h-1', action: 'ALERT_CREATED', performedBy: 'AlertEngine', timestamp: new Date(Date.now() - 1200000).toISOString(), comment: 'Incident generated via O(1) fingerprint recognition.' },
        { id: 'h-2', action: 'DEDUPLICATED_FOLDING', performedBy: 'RuleEngine', timestamp: new Date(Date.now() - 600000).toISOString(), comment: 'Recurring spike folded into occurrence #5.' },
      ]);
      setComments(res.comments || [
        { id: 'c-1', author: 'System Operator (L1)', content: 'Investigating high CPU utilization on target DB cluster node.', createdAt: new Date(Date.now() - 300000).toISOString() }
      ]);
    } catch (err: any) {
      setError(err.message || 'Failed to retrieve deep-dive incident detail profile.');
    } finally {
      setLoading(false);
    }
  }, [alertId]);
  ...
  const handleStatusChange = async (newStatus: string) => {
    try {
      await alertApi.updateStatus(alertId, newStatus, `Operator changed status to ${newStatus}`);
  ...
  const submitComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setComments((prev) => [
        ...prev,
        { id: `c-${Date.now()}`, author: 'Current Operator', content: triageNote, createdAt: new Date().toISOString() }
      ]);
      setTriageNote('');
    } finally {
  ...
  if (loading && !alertData) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-slate-400 font-sans space-y-4">
        ...
        <span className="text-sm font-semibold font-mono tracking-widest uppercase">Loading Incident SLA Workspace...</span>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-slate-800 pb-5">
        ...
        <div className="flex items-center space-x-4">
          <span className="text-[11px] font-mono font-semibold px-3 py-1 rounded-full bg-slate-900 border border-slate-800 text-slate-300">
            SLA Escalation Worker: <span className="text-emerald-400 font-extrabold">MONITORED</span>
          </span>
          <RealtimeStatusBadge />
        </div>
      </div>
        ...
        <div className="p-6 md:p-8 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-2xl space-y-6 relative overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
            <div className="space-y-3">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="inline-flex items-center px-3 py-1 rounded-lg text-xs font-extrabold bg-rose-950 text-rose-300 border border-rose-600 shadow animate-pulse">
                  <Flame className="w-3.5 h-3.5 mr-1 text-rose-400" /> {alertData.severity || 'CRITICAL'} PRIORITY
                </span>
              ...
              <h1 className="text-3xl font-extrabold text-white tracking-tight">{alertData.title || 'Anomalous Resource Utilization Spike'}</h1>
              <p className="text-sm font-mono text-slate-300 max-w-3xl leading-relaxed">{alertData.description || 'System monitored metrics breached hard enterprise operational tolerance thresholds.'}</p>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-slate-950 border border-slate-800 text-center min-w-[220px]">
              <span className="text-[11px] font-bold uppercase tracking-widest text-slate-400 block">Enterprise Risk Score</span>
              <div className="text-4xl font-black font-mono text-rose-400 mt-1.5">{alertData.riskScore || 95}<span className="text-lg text-slate-600 font-normal">/100</span></div>
              <span className="text-xs font-bold text-rose-500 mt-1 block">Immediate SLA Action Required</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-slate-800 text-xs font-mono">
            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-950 border border-slate-800/80">
              ...
              <Link href={`/device/${alertData.deviceId}`} className="text-sky-400 font-bold hover:underline flex items-center gap-1">
                <Server className="w-3.5 h-3.5" />
                <span>{alertData.deviceId || 'srv-prod-db01'}</span>
              </Link>
            </div>
            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-950 border border-slate-800/80">
              <span className="text-slate-400 font-semibold">O(1) SHA256 Fingerprint:</span>
              <button onClick={copyFingerprint} className="text-emerald-400 font-bold hover:text-emerald-300 inline-flex items-center gap-1" title="Copy exact hash">
                <span>{alertData.fingerprint ? `${alertData.fingerprint.slice(0, 14)}...` : 'e3b0c44298fc1c...'}</span>
                {copiedHash ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5 text-slate-500" />}
              </button>
            </div>
            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-950 border border-slate-800/80">
              <span className="text-slate-400 font-semibold">Deduplication Folding:</span>
              <span className="text-amber-400 font-extrabold">Occured {alertData.occurrenceCount || 5}x across fleet</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-6">
          <div className="p-6 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl space-y-5">
            ...
            <p className="text-xs text-slate-400">Execute instantaneous state transitions or scheduled cooldown suppression windows.</p>
            ...
            <div className="pt-4 border-t border-slate-800 space-y-2">
              <span className="text-[11px] font-bold uppercase tracking-wider text-indigo-400 block">Snooze / Cooldown Window</span>
              <div className="grid grid-cols-3 gap-2 text-center text-xs font-mono">
                <button onClick={() => handleSnooze(15)} className="p-2 rounded-lg bg-indigo-950/70 border border-indigo-800 hover:bg-indigo-900 font-bold text-indigo-200">
                  15 Min
                </button>
                <button onClick={() => handleSnooze(60)} className="p-2 rounded-lg bg-indigo-950/70 border border-indigo-800 hover:bg-indigo-900 font-bold text-indigo-200">
                  1 Hour
                </button>
                <button onClick={() => handleSnooze(240)} className="p-2 rounded-lg bg-indigo-950/70 border border-indigo-800 hover:bg-indigo-900 font-bold text-indigo-200">
                  4 Hours
                </button>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-950 border border-slate-800/80 space-y-2">
              ...
              <p className="text-[11px] text-slate-400 leading-relaxed">
                If open unacknowledged for <strong className="text-white">&gt;20m</strong>, escalated to Senior Admin pools. At <strong className="text-rose-400">&gt;40m</strong>, automatically promoted to CRITICAL severity.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="p-6 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-2xl space-y-6">
            <div className="border-b border-slate-800 pb-4">
              ...
              <p className="text-xs text-slate-400 mt-0.5">Every detection occurrence, deduction fold, notification receipt, and operator comment is immutably recorded.</p>
            </div>
            ...
            <div className="space-y-4 pt-4 border-t border-slate-800">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Chronological Event Feed ({history.length + comments.length})</h3>
                  ...
                  <div key={c.id} className="pl-8 relative group">
                    ...
                    <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
                      <div className="flex justify-between items-center text-[11px]">
                        <span className="font-extrabold text-sky-300 font-mono">{c.author || 'System Operator'} (Triage Note)</span>
                        <span className="text-slate-500 font-mono">{new Date(c.createdAt || Date.now()).toLocaleTimeString()}</span>
                      </div>
                      <p className="text-xs text-slate-200 font-sans mt-1 leading-relaxed">{c.content}</p>
                    </div>
                  </div>
                ))}
                ...
                {history.map((h) => (
                  <div key={h.id} className="pl-8 relative group">
                    ...
                    <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800/60 space-y-1">
                      <div className="flex justify-between items-center text-[11px]">
                        <span className="font-bold text-slate-300 font-mono uppercase">{h.action || 'AUDIT_EVENT'} • <span className="text-slate-400 font-normal">By {h.performedBy || 'System'}</span></span>
                        <span className="text-slate-500 font-mono">{new Date(h.timestamp || Date.now()).toLocaleTimeString()}</span>
                      </div>
                      <p className="text-xs text-slate-400 font-mono mt-0.5">{h.comment}</p>
                    </div>
                  </div>
                ))}
              </div>
