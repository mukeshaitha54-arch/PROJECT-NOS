...
export default function RegistrationKeysPage() {
  ...
  const handleRevoke = async (id: string) => {
    if (
      !confirm(
        "Are you sure you want to revoke this registration key? It cannot be undone.",
      )
    )
      return;
    try {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Manage agent enrollment tokens and their usage analytics.
          </p>
        </div>
        <Button
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="font-medium text-white">
                        {k.displayName || "Registration Key"}
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex-1 h-2 bg-gray-800 rounded-full overflow-hidden w-24">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, (k.currentUses / k.maxUses) * 100)}%`,
                            }}
                          />
                        </div>
                        <span className="text-xs font-mono">
                          {k.currentUses}/{k.maxUses}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      {k.createdByUser?.firstName} {k.createdByUser?.lastName}
                    </td>
                    <td className="px-6 py-4">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => {
                            ...
                            link.href =
                              "http://localhost:4000/api/v1/device/download";
                            link.setAttribute(
                              "download",
                              "NOS_Agent_Installer.exe",
                            );
