...
function DeviceRosterPageContent() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      setError(
        err?.message || "Failed to retrieve real-time monitoring agent status.",
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Enterprise Monitoring Agent Registry & 30-Second Diagnostic
                Health Verification
              </p>
            </div>
          </div>
        ...
        <div className="p-4 rounded-2xl bg-gradient-to-r from-blue-950/40 via-slate-900 to-cyan-950/40 border border-blue-500/30 text-xs sm:text-sm text-slate-300 flex items-center gap-3.5">
          ...
          <p>
            <span className="font-semibold text-white">
              Zero-Trust Agent Verification Domain:
            </span>{" "}
            Displays authenticated machine enrollment, hardware profile, UUIDs,
            and live 30s heartbeats. High-volume network packet telemetry
            collection and predictive AI chart analytics remain isolated until
            upcoming Phase 2B/3 activation.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl relative overflow-hidden">
            ...
            <p className="text-[11px] text-slate-500 mt-2 flex items-center gap-1">
              ...
              Cryptographic tokens provisioned
            </p>
          </div>
          ...
          <div className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl relative overflow-hidden">
            ...
            <p className="text-[11px] text-slate-500 mt-2 flex items-center gap-1">
              <Activity className="w-3.5 h-3.5 text-green-400" /> Active within
              last 90 seconds
            </p>
          </div>
          ...
          <div className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl relative overflow-hidden">
            ...
            <p className="text-[11px] text-slate-500 mt-2 flex items-center gap-1">
              <Clock className="w-3.5 h-3.5 text-slate-500" /> Missed heartbeat
              window
            </p>
          </div>
          ...
          <div className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl relative overflow-hidden">
            ...
            <p className="text-[11px] text-slate-500 mt-2 flex items-center gap-1">
              <Activity className="w-3.5 h-3.5 text-amber-400" /> Resource
              constraint or diagnostic warning
            </p>
          </div>
        </div>
          ...
          <div className="p-6 border-b border-slate-800/80 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Real-time status updates via persistent 30s worker diagnostic
                heartbeats
              </p>
            </div>
              ...
              <thead className="bg-slate-950/80 text-[11px] font-semibold uppercase tracking-wider text-slate-400 border-b border-slate-800/80">
                <tr>
                  ...
                  <th className="py-3.5 px-6">
                    Live Diagnostic Heartbeat (CPU / RAM)
                  </th>
                </tr>
              </thead>
                    ...
                    >
                      <div className="max-w-md mx-auto space-y-2">
                        ...
                        <p className="text-sm font-medium text-slate-300">
                          No monitoring agents registered yet.
                        </p>
                        <p className="text-xs text-slate-500">
                          Launch the .NET 8 Worker daemon (`NOS.Agent.exe`) to
                          provision hardware credentials and begin 30-second
                          heartbeat polling.
                        </p>
                      </div>
                    </td>
                      ...
                      >
                        <td className="py-4 px-6">
                          <Link
                            ...
                            className="font-bold text-white hover:text-cyan-400 transition-colors flex items-center gap-1.5 text-sm underline-offset-4 hover:underline"
                          >
                        ...
                        <td className="py-4 px-6 font-mono text-xs text-slate-400">
                          <div
                            className="p-2 rounded-lg bg-slate-950/80 border border-slate-800/80 max-w-[220px] truncate"
                            title={device.uuid}
                          >
                            {device.uuid}
                          </div>
                            ...
                            <div className="space-y-2 max-w-xs">
                              ...
                              <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden flex">
                                <div
                                  ...
                                  style={{
                                    width: `${Math.min(100, Math.max(0, hb.cpuUsage))}%`,
                                  }}
                                />
                              </div>
...
const DynamicDeviceRosterPage = nextDynamic(
  () => Promise.resolve(DeviceRosterPageContent),
  { ssr: false },
);
