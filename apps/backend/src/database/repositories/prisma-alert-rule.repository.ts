...
export class PrismaAlertRuleRepository implements IAlertRuleRepository {
  ...
  async findManyPaginated(query: AlertRuleFindManyQuery): Promise<[AlertRule[], number]> {
    const where: Prisma.AlertRuleWhereInput = {};
  ...
  async clone(id: string, newName: string, performedBy: string): Promise<AlertRule> {
    const source = await this.prisma.alertRule.findUniqueOrThrow({ where: { id } });
    return this.prisma.alertRule.create({
      data: {
        version: 1,
        name: newName,
        description: source.description ? `Clone of: ${source.description}` : `Cloned from "${source.name}"`,
  ...
  async updatePerformanceMetrics(id: string, metrics: {
    ...
    const current = await this.prisma.alertRule.findUnique({
      where: { id },
      select: { evaluationCount: true, avgExecMs: true, maxExecMs: true, minExecMs: true },
    });
    if (!current) return;

    const evalCount = Number(current.evaluationCount);
    const currentAvg = current.avgExecMs;
    const newAvg = evalCount === 0
      ? metrics.execMs
    ...
    await this.prisma.alertRule.update({
      ...
      data: {
        ...
        avgExecMs: newAvg,
        maxExecMs: metrics.execMs > (current.maxExecMs || 0) ? metrics.execMs : undefined,
        minExecMs: current.minExecMs === 0 || metrics.execMs < current.minExecMs
          ? metrics.execMs
          : undefined,
  ...
  async findConflicting(): Promise<Array<{ ruleIds: string[]; reason: string }>> {
    ...
    for (let i = 0; i < rules.length; i++) {
      for (let j = i + 1; j < rules.length; j++) {
        const a = rules[i];
        const b = rules[j];
        if (a.metric === b.metric && a.operator === b.operator && a.threshold === b.threshold && a.severity !== b.severity) {
          conflicts.push({
            ruleIds: [a.id, b.id],
  ...
  async getTags(): Promise<string[]> {
    const results = await this.prisma.alertRule.findMany({ select: { tags: true } });
