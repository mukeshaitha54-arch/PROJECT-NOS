...
export interface IAlertRuleAuditRepository {
  ...
  findByRuleId(ruleId: string, skip?: number, take?: number): Promise<[AlertRuleAuditLog[], number]>;
  findByAction(action: string, skip?: number, take?: number): Promise<[AlertRuleAuditLog[], number]>;
  findByPerformedBy(performedBy: string, skip?: number, take?: number): Promise<[AlertRuleAuditLog[], number]>;
}
