...
import {
  ...
  DeviceCredentials,
} from "./device-authenticator.interface";
...
export class TokenDeviceAuthenticatorService implements IDeviceAuthenticator {
  ...
  async authenticate(
    ...
    const deviceTokenHeader =
      headers["x-device-token"] || headers["X-Device-Token"];

    if (
      typeof deviceTokenHeader === "string" &&
      deviceTokenHeader.trim().length > 0
    ) {
    ...
    if (!token) {
      this.logger.debug(
        "No valid Device Token or Authorization header present on incoming heartbeat request.",
      );
      return null;
    }
    ...
    if (!device) {
      this.logger.warn(
        `Unrecognized device token presented during authentication verification.`,
      );
      return null;
    }
