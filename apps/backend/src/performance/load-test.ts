...
function runPerformanceBenchmark(): Promise<autocannon.Result> {
  ...
  console.log(
    "Simulating 100 concurrent agent connections over 10 seconds...\n",
  );
  ...
  return new Promise((resolve, reject) => {
    const instance = autocannon(
      ...
      (err, result) => {
        ...
        console.log(
          `Throughput (bytes/sec): ${(result.throughput.average / 1024 / 1024).toFixed(2)} MB/s`,
        );
        ...
        if (result.latency.p99 > 200) {
          console.warn(
            "⚠️ Warning: p99 latency exceeded target threshold of 200ms.",
          );
        } else {
          console.log(
            "✅ Performance verification PASSED: p99 latency < 200ms target satisfied.",
          );
        }
