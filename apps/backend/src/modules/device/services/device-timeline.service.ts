...
export class DeviceTimelineService {
  ...
  async logEvent(dto: CreateTimelineEventDto): Promise<TimelineEventDto> {
    try {
      ...
      this.logger.debug(`[Timeline] Logged ${dto.eventType} for device ${dto.deviceId}: "${dto.title}"`);
      return event;
    } catch (err: any) {
      this.logger.error(`[Timeline] Failed to log event for device ${dto.deviceId}: ${err?.message}`);
      throw err;
    }
  }

  async getPaginatedTimeline(options: TimelineQueryOptions): Promise<PaginatedTimelineResponse> {
    return this.timelineRepo.getPaginated(options);
  }

  async getRecentTimeline(deviceId: string, limit = 10): Promise<TimelineEventDto[]> {
