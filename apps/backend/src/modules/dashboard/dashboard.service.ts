...
export class DashboardService {
  ...
  constructor(
    @Inject(IDashboardRepository) private readonly repository: IDashboardRepository,
  ...
  async getDeviceById(organizationId: string, id: string): Promise<DashboardDeviceDetailResponse> {
    ...
    const currentCpu = currentSnapshot ? Math.round(currentSnapshot.cpuUsage * 10) / 10 : (latestHeartbeat ? Math.round(latestHeartbeat.cpuUsage * 10) / 10 : 0);
    const currentRam = currentSnapshot ? Math.round(currentSnapshot.memoryUsagePercent * 10) / 10 : (latestHeartbeat ? Math.round(latestHeartbeat.ramUsage * 10) / 10 : 0);
    const currentDisk = currentSnapshot ? Math.round(currentSnapshot.diskUsagePercent * 10) / 10 : 0;
    
    const currentNetwork = {
      uploadSpeed: currentSnapshot ? Math.round(currentSnapshot.networkUploadSpeed || 0) : 0,
      downloadSpeed: currentSnapshot ? Math.round(currentSnapshot.networkDownloadSpeed || 0) : 0,
      bytesSent: currentSnapshot ? currentSnapshot.bytesSent : 0,
      bytesReceived: currentSnapshot ? currentSnapshot.bytesReceived : 0,
      activeConnections: currentSnapshot ? (currentSnapshot.activeConnections || 0) : 0,
      ipAddress: currentSnapshot ? currentSnapshot.ipAddress : (latestHeartbeat ? latestHeartbeat.ipAddress : '0.0.0.0'),
      macAddress: currentSnapshot ? currentSnapshot.macAddress : '00:00:00:00:00:00',
    };

    const uptime = currentSnapshot ? currentSnapshot.systemUptime : (latestHeartbeat ? latestHeartbeat.uptime : 0);
    const deviceStatus = device.status as DeviceStatus;

    let systemStatus = 'HEALTHY';
    if (deviceStatus === DeviceStatus.CRITICAL) {
      systemStatus = 'CRITICAL';
    } else if (deviceStatus === DeviceStatus.DEGRADED || currentCpu > 85 || currentRam > 88 || currentDisk > 92) {
    ...
    const cleanedDevice = {
      ...
      registeredAt: device.registeredAt ? device.registeredAt.toISOString() : new Date(0).toISOString(),
      createdAt: device.createdAt ? device.createdAt.toISOString() : new Date(0).toISOString(),
      updatedAt: device.updatedAt ? device.updatedAt.toISOString() : new Date(0).toISOString(),
      tokenHash: undefined, // scrub internal auth hashes
    };

    return {
      device: cleanedDevice,
      currentSnapshot: currentSnapshot ? this.toTelemetrySnapshotDto(currentSnapshot) : null,
      latestHeartbeat: latestHeartbeat ? {
        ...
        timestamp: latestHeartbeat.timestamp instanceof Date ? latestHeartbeat.timestamp.toISOString() : String(latestHeartbeat.timestamp),
      } : null,
      currentCpu,
      currentRam,
      currentDisk,
  ...
  async getDeviceHistory(organizationId: string, deviceId: string, query: DashboardHistoryQueryDto): Promise<PaginatedTelemetryResponse> {
    ...
    const dtos: TelemetrySnapshot[] = snapshots.map((s) => this.toTelemetrySnapshotDto(s));
  ...
  private toTelemetrySnapshotDto(record: any): TelemetrySnapshot {
    return {
      ...
      bootTime: record.bootTime instanceof Date ? record.bootTime.toISOString() : String(record.bootTime),
