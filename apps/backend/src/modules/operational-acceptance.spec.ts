...
import { ITelemetryPublisherToken, ITelemetryPublisher } from '../common/services/telemetry-publisher.interface';
...
import { Device as PrismaDevice, TelemetrySnapshot as PrismaTelemetrySnapshot } from '@prisma/client';
...
describe('Operational Acceptance Test (OAT) Suite — Pre-Phase 7 Operational Readiness', () => {
  ...
  beforeEach(async () => {
    ...
    const mockDeviceRepo: Partial<IDeviceRepository> = {
      ...
      findById: jest.fn().mockImplementation(async (id) => deviceStore.get(id) || null),
      findByUuid: jest.fn().mockImplementation(async (uuid) => Array.from(deviceStore.values()).find((d) => d.uuid === uuid) || null),
    ...
    const mockHeartbeatRepo: Partial<IHeartbeatRepository> = {
      ...
      findLatestByDeviceId: jest.fn().mockImplementation(async (deviceId) => heartbeatStore.get(deviceId) || null),
    };
    ...
    const mockTelemetryRepo: Partial<ITelemetryRepository> = {
      ...
      findRange: jest.fn().mockImplementation(async (query) => {
        const matches = telemetryStore.filter((t) => t.deviceId === query.deviceId);
        return { items: matches, total: matches.length };
      }),
    };

    const mockInventoryRepo: Partial<IInventoryRepository> = {
      upsertInventory: jest.fn().mockImplementation(async (deviceId, payload) => {
        const inv = {
          deviceId,
      ...
      findCompleteInventory: jest.fn().mockImplementation(async (deviceId) => inventoryStore.get(deviceId) || null),
    ...
    const mockAuditRepo: Partial<IAuditLogRepository> = {
      record: jest.fn().mockImplementation(async (log) => {
        const entry = { id: `audit-${Date.now()}`, ...log, timestamp: new Date().toISOString() };
        auditStore.push(entry);
        return entry as any;
      }),
      search: jest.fn().mockImplementation(async (req) => {
        const matches = auditStore.filter((a) => (req.organizationId ? a.organizationId === req.organizationId : true));
        return { items: matches, total: matches.length, page: 1, limit: 50, totalPages: 1 };
      }),
    };

    const mockOrgRepo: Partial<IOrganizationRepository> = {
      findById: jest.fn().mockImplementation(async (id) => orgStore.get(id) || null),
    };
    ...
    const mockHeartbeatPresence: Partial<HeartbeatPresenceService> = {
      processHeartbeat: jest.fn().mockImplementation(async (deviceId, ipAddress, cpuUsage, ramUsage, uptime, correlationId) => {
        await mockSocketPublisher.emitHeartbeatReceived(deviceId, {
          deviceId,
          cpuUsage,
          ramUsage,
          uptime,
    ...
    const mockInventoryAudit: Partial<InventoryAuditService> = {
      calculateAssetFingerprint: jest.fn().mockReturnValue('mock_fingerprint_hash'),
      detectAndLogDifferences: jest.fn().mockResolvedValue([]),
    };

    const mockTimelineRepo = {
      append: jest.fn().mockImplementation((dto) => Promise.resolve({ id: 'evt-mock-1', ...dto, timestamp: new Date().toISOString() })),
      getPaginated: jest.fn().mockResolvedValue({ events: [], total: 0, page: 1, limit: 20, totalPages: 1 }),
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: RegistrationKeyService, useValue: { validateKey: jest.fn().mockResolvedValue({ id: 'key-test-1', organizationId: orgAlphaContext.organizationId }), incrementKeyUsage: jest.fn().mockResolvedValue(true) } },
        { provide: PrismaService, useValue: { inventorySnapshot: { findUnique: jest.fn().mockResolvedValue(null), findFirst: jest.fn().mockResolvedValue(null), findMany: jest.fn().mockResolvedValue([]), create: jest.fn(), update: jest.fn(), upsert: jest.fn() }, device: { findUnique: jest.fn().mockResolvedValue(null), findFirst: jest.fn().mockResolvedValue(null) }, $transaction: jest.fn().mockImplementation(cb => cb({})) } },
      ],
    }).compile();
  ...
  it('Module 2 DoD Verification: Registers via Registration Key, issues JWT/token, emits WebSocket online event, ingests heartbeat, and recovers session', async () => {
    ...
    expect(registration.registrationToken).toBe('mock_raw_device_token_secret_123');
    expect(registration.device.status).toBe(DeviceStatus.ONLINE);
    expect(registration.device.organizationId).toBe(orgAlphaContext.organizationId);

    // Step 7 & 8: Appear in Fleet page & Show Online (via real-time Socket emissions & status API)
    expect(mockSocketPublisher.emitDeviceConnected).toHaveBeenCalledWith(registration.deviceId, expect.objectContaining({
      id: registration.deviceId,
      hostname: 'PROD-MOD2-WIN-PC',
      status: DeviceStatus.ONLINE,
    }));
    expect(mockSocketPublisher.emitDeviceOnline).toHaveBeenCalledWith(registration.deviceId, expect.objectContaining({
      deviceId: registration.deviceId,
      status: 'ONLINE',
    }));

    const fleetStatus = await deviceService.getPlatformStatus();
    const enrolled = fleetStatus.devices.find(d => d.id === registration.deviceId);
    ...
    const hbResponse = await deviceService.recordHeartbeat(deviceStore.get(registration.deviceId)!, hbDto);
    ...
    const profile = await deviceService.getDeviceProfile(deviceStore.get(registration.deviceId)!);
  ...
  it('OAT Checkpoint 4: Ingests and retrieves live CPU, RAM, Disk, and Network telemetry', async () => {
    ...
    expect(mockSocketPublisher.emitTelemetryReceived).toHaveBeenCalledWith(registration.deviceId, expect.anything());
  });
  ...
  it('OAT Checkpoint 7: Persists and views complete hardware, software, network, and security inventory', async () => {
    ...
    const fullInventory: SubmitInventoryPayload = {
      ...
      osBuild: '20348',
      architecture: 'X64',
      memoryModules: [{ slot: 'DIMM_A1', capacityBytes: 68719476736, speedMHz: 3200, manufacturer: 'Samsung', partNumber: 'M393A8G40AB2', serialNumber: 'S001' }],
      diskDrives: [{ driveName: 'C:', model: 'Samsung NVMe 980', serialNumber: 'S1234', mediaType: 'NVMe', sizeBytes: 536870912000, fileSystem: 'NTFS', isSystemDrive: true }],
      gpus: [{ name: 'NVIDIA RTX A4000', manufacturer: 'NVIDIA', driverVersion: '535.104', vRamBytes: 17179869184, resolution: '3840x2160' }],
      networkAdapters: [{ name: 'Ethernet 0', macAddress: '00:1A:2B:3C:4D:62', ipv4: '10.0.1.29', ipv6: 'fe80::1', gateway: '10.0.1.1', dns: '8.8.8.8', speedMbps: 10000, isWireless: false, isPhysical: true, isOperational: true }],
      installedSoftware: [{ name: 'NOS Monitoring Agent', version: '1.0.0', publisher: 'NOS', installDate: '2026-01-01' }],
      windowsServices: [{ serviceName: 'nos-agent', displayName: 'NOS Agent Daemon', status: 'RUNNING', startType: 'AUTOMATIC', account: 'LocalSystem' }],
      startupApplications: [{ name: 'NOSAgentTray', command: 'C:\\Program Files\\NOS\\tray.exe', location: 'HKLM\\Run', user: 'SYSTEM' }],
      security: {
    ...
    await inventoryService.submitInventory(fullInventory, registration.deviceId);

    const retrieved = await inventoryService.getCompleteInventory(registration.deviceId);
  ...
  it('OAT Checkpoint 8: Verifies audit trail creation during operations', async () => {
    ...
    const searchResult = await auditService.search({ organizationId: orgAlphaContext.organizationId });
    expect(searchResult.items.length).toBe(1);
    expect(searchResult.items[0].resourceId).toBe('dev-oat-100');
    expect(searchResult.items[0].organizationId).toBe(orgAlphaContext.organizationId);
  });
  ...
  it('OAT Checkpoint 9: Guarantees 0% cross-tenant data leakage between Org-Alpha and Org-Beta', async () => {
    ...
    const alphaDevices = all.devices.filter((d) => d.organizationId === orgAlphaContext.organizationId);
    ...
    const betaDevices = all.devices.filter((d) => d.organizationId === orgBetaContext.organizationId);
  ...
  it('OAT Checkpoint 11: Sets device in maintenance mode and retires device with audit logging', async () => {
    ...
    const updatedMaint = await deviceService.setMaintenanceMode(reg.deviceId, true, 'user-admin-01', 'Admin Operator');
    expect(updatedMaint.status).toBe(DeviceStatus.MAINTENANCE);

    // Toggle Maintenance Mode OFF
    const updatedOnline = await deviceService.setMaintenanceMode(reg.deviceId, false, 'user-admin-01', 'Admin Operator');
    expect(updatedOnline.status).toBe(DeviceStatus.ONLINE);

    // Retire Device
    const retired = await deviceService.retireDevice(reg.deviceId, 'user-admin-01', 'Admin Operator');
  ...
  it('OAT Checkpoint 12: Executes bulk status updates across multiple monitored nodes', async () => {
    ...
    const result = await deviceService.bulkUpdateStatus([reg1.deviceId, reg2.deviceId], DeviceStatus.MAINTENANCE);
