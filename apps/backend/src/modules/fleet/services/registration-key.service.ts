...
import {
  ...
  IRegistrationKeyRepository as RegistrationKeyRepoToken,
} from "../../../common/repositories/registration-key.repository.interface";
import {
...
export class RegistrationKeyService {
  ...
  async validateKey(
    ...
    if (
      registrationKey.maxUses > 0 &&
      registrationKey.currentUses >= registrationKey.maxUses
    ) {
      await this.registrationKeyRepo.recordFailedAttempt(registrationKey.id);
