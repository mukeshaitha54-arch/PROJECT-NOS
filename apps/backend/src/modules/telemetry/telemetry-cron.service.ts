...
export class TelemetryCronService {
  ...
  async handle1MinuteAggregation(): Promise<void> {
    ...
    if (!(await this.acquireLock(lockKey))) {
      this.logger.warn('[1m] Telemetry aggregation already RUNNING. Skipping scheduled run.');
      return;
    }

    try {
      const startTime = new Date();
      this.logger.log(`Starting [1m] aggregation schedule at ${startTime.toISOString()}`);
      ...
      this.logger.error(`Failed [1m] scheduled aggregation: ${error instanceof Error ? error.stack : error}`);
    } finally {
  ...
  async handle15MinuteAggregation(): Promise<void> {
    ...
    if (!(await this.acquireLock(lockKey))) {
      this.logger.warn('[15m] Telemetry aggregation already RUNNING. Skipping scheduled run.');
      return;
    }

    try {
      const startTime = new Date();
      this.logger.log(`Starting [15m] aggregation schedule at ${startTime.toISOString()}`);
      
      const result = await this.aggregationService.aggregateSnapshots('15m', 30);
      ...
      this.logger.error(`Failed [15m] scheduled aggregation: ${error instanceof Error ? error.stack : error}`);
    } finally {
      await this.releaseLock(lockKey);
    }
  }
  ...
  async handle1HourAggregation(): Promise<void> {
    ...
    if (!(await this.acquireLock(lockKey))) {
      this.logger.warn('[1h] Telemetry aggregation already RUNNING. Skipping scheduled run.');
      return;
    }

    try {
      const startTime = new Date();
      this.logger.log(`Starting [1h] aggregation schedule at ${startTime.toISOString()}`);
      
      const result = await this.aggregationService.aggregateSnapshots('1h', 360);
      ...
      this.logger.error(`Failed [1h] scheduled aggregation: ${error instanceof Error ? error.stack : error}`);
    } finally {
      await this.releaseLock(lockKey);
    }
  }
  ...
  async handleDailyAggregation(): Promise<void> {
    ...
    if (!(await this.acquireLock(lockKey))) {
      this.logger.warn('[1d] Telemetry aggregation already RUNNING. Skipping scheduled run.');
      return;
    }

    try {
      const startTime = new Date();
      this.logger.log(`Starting [1d] daily aggregation schedule at ${startTime.toISOString()}`);
      
      const result = await this.aggregationService.aggregateSnapshots('1d', 1440);
      ...
      this.logger.error(`Failed [1d] scheduled aggregation: ${error instanceof Error ? error.stack : error}`);
    } finally {
      await this.releaseLock(lockKey);
    }
  }
  ...
  async handleDailyRetentionPurge(): Promise<void> {
    ...
    if (!(await this.acquireLock(lockKey))) {
      this.logger.warn('Telemetry data retention purge already RUNNING. Skipping scheduled run.');
      return;
    }

    try {
      this.logger.log(`Triggering daily data retention purge schedule at ${new Date().toISOString()}...`);
      await this.retentionService.purgeOldData();
    } catch (error) {
      this.logger.error(`Failed daily retention purge: ${error instanceof Error ? error.stack : error}`);
    } finally {
      await this.releaseLock(lockKey);
    }
  }
  ...
  private async acquireLock(key: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`Error acquiring KeyValue lock [${key}]: ${error instanceof Error ? error.message : error}`);
  ...
  private async releaseLock(key: string): Promise<void> {
    try {
      ...
      this.logger.error(`Error releasing KeyValue lock [${key}]: ${error instanceof Error ? error.message : error}`);
    }
  }
}
