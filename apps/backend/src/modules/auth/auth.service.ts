import { Injectable, UnauthorizedException, BadRequestException, ConflictException, NotFoundException, Inject, Logger } from '@nestjs/common';
import { JwtService } from '@nestjs/jwt';
import { ConfigService } from '@nestjs/config';
import { User, TokenResponsePayload, ErrorCode, UserRole } from '@nos/shared-types';
import { IUserRepository, IUserRepositoryToken } from '../../common/repositories/user.repository.interface';
import { ITokenRepository, ITokenRepositoryToken } from '../../common/repositories/token.repository.interface';
...
export class AuthService {
  ...
  async register(dto: RegisterDto): Promise<{ message: string; user: User }> {
    ...
    if (existing) {
      throw new ConflictException({ code: ErrorCode.USER_ALREADY_EXISTS, message: 'Account with this email already exists.' });
    }
    ...
    await this.tokenRepo.createOtp(user.id, user.email, otpHash, 'EMAIL_VERIFY', expiresAt);
    await this.mailService.sendVerificationOtp(user.email, otp);

    const { passwordHash: _, ...cleanUser } = user;
    this.logger.log(`👤 New user registered: [${cleanUser.email}] (ID: ${cleanUser.id})`);
    return { message: 'Registration successful. Please check your email for the verification OTP.', user: cleanUser };
  }
  ...
  async login(dto: LoginDto): Promise<TokenResponsePayload> {
    ...
    if (!user.isEmailVerified && this.configService.get<string>('REQUIRE_EMAIL_VERIFICATION') === 'true') {
      throw new UnauthorizedException({ code: ErrorCode.EMAIL_NOT_VERIFIED, message: 'Email address has not been verified.' });
    }
  ...
  async verifyEmail(dto: VerifyEmailDto): Promise<{ message: string }> {
    const latestOtp = await this.tokenRepo.findLatestOtp(dto.email, 'EMAIL_VERIFY');
    if (!latestOtp) {
      throw new BadRequestException({ code: ErrorCode.INVALID_OTP, message: 'No valid verification request found for this email.' });
    }

    if (new Date() > latestOtp.expiresAt) {
      throw new BadRequestException({ code: ErrorCode.OTP_EXPIRED, message: 'Verification OTP has expired. Please request a new one.' });
    }

    if (!(await this.hasher.verify(latestOtp.otpHash, dto.otp))) {
      throw new BadRequestException({ code: ErrorCode.INVALID_OTP, message: 'Incorrect OTP code.' });
    }
  ...
  async refresh(dto: RefreshDto): Promise<TokenResponsePayload> {
    ...
    const { passwordHash: _, ...cleanUser } = user;
    this.logger.log(`🔄 Rotated session refresh tokens for user: [${cleanUser.email}]`);
    return this.generateTokens(cleanUser);
  }

  async logout(user: User): Promise<{ message: string }> {
    await this.tokenRepo.revokeAllUserRefreshTokens(user.id);
    this.logger.log(`🚪 User logged out and session tokens revoked: [${user.email}]`);
  ...
  async forgotPassword(dto: ForgotPasswordDto): Promise<{ message: string }> {
    ...
    if (user) {
      ...
      await this.tokenRepo.createOtp(user.id, user.email, otpHash, 'PASSWORD_RESET', expiresAt);
      await this.mailService.sendPasswordResetOtp(user.email, otp);
      this.logger.log(`📩 Password reset OTP generated for: [${user.email}]`);
    }
    return { message: 'If an account matching this email exists, a password reset OTP has been sent.' };
  }

  async resetPassword(dto: ResetPasswordDto): Promise<{ message: string }> {
    const latestOtp = await this.tokenRepo.findLatestOtp(dto.email, 'PASSWORD_RESET');
    if (!latestOtp) {
      throw new BadRequestException({ code: ErrorCode.INVALID_OTP, message: 'No active password reset request found.' });
    }

    if (new Date() > latestOtp.expiresAt) {
      throw new BadRequestException({ code: ErrorCode.OTP_EXPIRED, message: 'Password reset OTP has expired.' });
    }
    ...
    const user = await this.userRepo.findByEmail(dto.email);
    if (!user) {
      throw new NotFoundException({ code: ErrorCode.USER_NOT_FOUND, message: 'Account not found.' });
    }
    ...
    return { message: 'Password reset successful. Please login with your new credentials.' };
  }
  ...
  async changePassword(user: User, dto: ChangePasswordDto): Promise<{ message: string }> {
    ...
    if (!fullUser || !(await this.hasher.verify(fullUser.passwordHash, dto.currentPassword))) {
    ...
    this.logger.log(`🔒 Password changed and existing sessions revoked for: [${user.email}]`);
