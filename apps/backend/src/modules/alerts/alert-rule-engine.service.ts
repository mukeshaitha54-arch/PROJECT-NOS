...
export class AlertRuleEngineService {
  ...
  async evaluateTelemetry(
    ...
    try {
      ...
      if (!device) {
        this.logger.warn(`Cannot evaluate telemetry for non-existent device ID [${deviceId}].`);
        return;
      }

      const deviceName = (device as any).name || device.deviceName || device.hostname || device.id;
      const isUnderMaintenance =
        device.status === (DeviceStatus.MAINTENANCE as any) || (device.status as string) === 'MAINTENANCE';
      ...
      for (const rule of rules) {
        ...
        if (rule.deviceIds && rule.deviceIds.length > 0 && !rule.deviceIds.includes(deviceId)) {
          continue;
        }
        ...
        const isBreaching = this.compareThreshold(value, rule.operator, rule.threshold);
        ...
        if (isBreaching) {
          if (!existingBreach) {
            ...
            this.logger.debug(`[${deviceName}] Initiated new breach state for rule [${rule.name}] (${value} vs threshold ${rule.threshold})`);
          } else {
            ...
            const requiredDurationSeconds =
              rule.duration && rule.duration > 0 ? rule.duration * 60 : rule.durationSeconds || 60;
            const elapsedSeconds = (now.getTime() - existingBreach.firstBreachAt.getTime()) / 1000;
          ...
          if (existingBreach) {
            ...
            this.logger.debug(`[${deviceName}] Resolved breach state for rule [${rule.name}] as value dropped below threshold.`);
          }
        }
      }
    } catch (error) {
      this.logger.error(`Error evaluating telemetry for device [${deviceId}]: ${error instanceof Error ? error.stack : error}`);
    }
  }
  ...
  private async createAlert(params: {
    ...
    const { rule, deviceName, deviceId, tenantId, value, isUnderMaintenance, now } = params;

    // Title: "{deviceName}: {rule.name}" (e.g., "WS-001: High CPU Usage")
    const title = `${deviceName}: ${rule.name}`;
    const durationMin = rule.duration || Math.round((rule.durationSeconds || 60) / 60) || 1;
    ...
    const fingerprint = crypto.createHash('sha256').update(`${deviceId}:${rule.metric}:${rule.id}`).digest('hex');
    ...
    const targetStatus = isUnderMaintenance ? (AlertStatus.SUPPRESS as any) : AlertStatus.NEW;
    ...
    this.logger.log(`Triggered Alert [${alert.incidentNumber}] (${alert.status}) on [${deviceName}]: ${message}`);

    if (!isUnderMaintenance) {
      if (this.socketGateway?.server) {
        this.socketGateway.server.to(`tenant-${tenantId}`).emit('alert:triggered', alert);
      }
    }
  ...
  public extractMetricValue(metric: string, telemetry: TelemetryPayload): number | null {
    ...
    if (key === 'ram' || key === 'memory' || key === 'memoryusagepercent') {
      return telemetry.ram ?? telemetry.memoryUsagePercent ?? telemetry.ramUsage ?? null;
    }
    if (key === 'disk' || key === 'diskusagepercent') {
      return telemetry.disk ?? telemetry.diskUsagePercent ?? null;
    }
    if (key === 'network' || key === 'networkspeed') {
      return telemetry.network ?? telemetry.networkUploadSpeed ?? telemetry.bytesReceived ?? null;
    }
  ...
  public compareThreshold(value: number, operator: string, threshold: number): boolean {
  ...
  async testRule(ruleId: string, deviceId: string, simulatedValue: number) {
    ...
    if (!rule) {
      return { wouldTrigger: false, reason: `Rule ${ruleId} not found.`, durationRequired: 0, currentDuration: 0 };
    }
    const wouldTrigger = this.compareThreshold(simulatedValue, rule.operator, rule.threshold);
    const durationRequired = rule.duration || Math.round((rule.durationSeconds || 60) / 60) || 1;
