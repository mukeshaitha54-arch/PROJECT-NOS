...
export interface INotificationProvider {
  ...
  send(payload: NotificationPayload): Promise<{ success: boolean; response: string }>;
}

export class EmailNotificationProvider implements INotificationProvider {
  readonly providerType = NotificationProvider.EMAIL;
  async send(payload: NotificationPayload): Promise<{ success: boolean; response: string }> {
    ...
    console.log(`[EmailProvider] Delivering to ${payload.recipient}: ${template}`);
...
export class SlackNotificationProvider implements INotificationProvider {
  ...
  async send(payload: NotificationPayload): Promise<{ success: boolean; response: string }> {
    console.log(`[SlackProvider] Webhook payload to #operations: ${payload.incidentNumber} - ${payload.title}`);
...
export class WebhookNotificationProvider implements INotificationProvider {
  ...
  async send(payload: NotificationPayload): Promise<{ success: boolean; response: string }> {
    console.log(`[WebhookProvider] HTTP POST to ${payload.webhookUrl || payload.recipient} for ${payload.incidentNumber}`);
...
export class TeamsNotificationProvider implements INotificationProvider {
  ...
  async send(payload: NotificationPayload): Promise<{ success: boolean; response: string }> {
    console.log(`[TeamsProvider] Sending Teams Card for ${payload.incidentNumber}`);
...
export class DiscordNotificationProvider implements INotificationProvider {
  ...
  async send(payload: NotificationPayload): Promise<{ success: boolean; response: string }> {
    console.log(`[DiscordProvider] Sending Discord Embed for ${payload.incidentNumber}`);
...
export class SmsNotificationProvider implements INotificationProvider {
  ...
  async send(payload: NotificationPayload): Promise<{ success: boolean; response: string }> {
    console.log(`[SmsProvider] Sending SMS alert to ${payload.recipient}: ${payload.incidentNumber}`);
...
export class PushNotificationProvider implements INotificationProvider {
  ...
  async send(payload: NotificationPayload): Promise<{ success: boolean; response: string }> {
    console.log(`[PushProvider] Sending Mobile Push Notification for ${payload.incidentNumber}`);
    return { success: true, response: 'Push Notification Sent' };
  }
}

export function renderNotificationTemplate(template: string, variables: Record<string, string | number>): string {
