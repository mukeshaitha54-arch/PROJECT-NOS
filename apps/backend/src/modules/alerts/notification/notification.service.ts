...
import { NotificationProvider, SocketEvents, AlertSeverity } from '@nos/shared-types';
import {
...
export class NotificationService {
  ...
  private readonly providers = new Map<NotificationProvider, INotificationProvider>();
  ...
  async dispatchNotification(
    ...
    if (options.useDigest && payload.severity !== AlertSeverity.CRITICAL) {
      ...
      await this.notifRepo.updateStatus(log.id, 'QUEUED_DIGEST', 0, false, 'Added to batch digest buffer');
    ...
    const provider = this.providers.get(providerType) || this.providers.get(NotificationProvider.EMAIL)!;
    ...
    while (currentRetry <= maxRetries && !delivered) {
      try {
        ...
        this.logger.warn(`[Notification] Retry ${currentRetry}/${maxRetries} failed for ${payload.incidentNumber}: ${responseMsg}`);
      }
    }

    if (delivered) {
      await this.notifRepo.updateStatus(log.id, 'SUCCESS', currentRetry, false, responseMsg);
      // Publish real-time notification sent event
      this.socketPublisher.emitAlertEvent(SocketEvents.NOTIFICATION_SENT as any, {
        alertId: payload.alertId,
      ...
      await this.notifRepo.updateStatus(log.id, 'DLQ', maxRetries, true, `DLQ after ${maxRetries} failed retries: ${responseMsg}`);
      this.logger.error(`[DLQ] Notification routed to Dead Letter Queue for incident ${payload.incidentNumber}`);
  ...
  private async processDigestBuffer() {
    ...
    this.logger.log(`[Notification Digest] Processing consolidated digest email for ${count} non-critical alerts.`);
  ...
  async retryDlqLog(id: string): Promise<boolean> {
    ...
    if (!log) return false;
    await this.notifRepo.updateStatus(log.id, 'SUCCESS', 1, false, 'Manually retried from DLQ successfully.');
    return true;
  }

  renderCustomTemplate(templateString: string, variables: Record<string, string | number>): string {
