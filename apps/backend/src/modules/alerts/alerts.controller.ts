...
export class AlertsController {
  ...
  async getActiveAlerts(@Request() req: any) {
    const tenantId =
      req.user?.organizationId || req.user?.tenantId || "default-org";
    const [all] = await this.alertEngine.getAlerts({ take: 500 });
    const active = all.filter((a) =>
      ["NEW", "OPEN", "ACKNOWLEDGED"].includes(a.status as string),
    );
    return {
      success: true,
      data: active,
  ...
  async getOverview() {
    ...
    const list = alerts[0];
    const openAlerts = list.filter(
      (a) => a.status === "NEW" || a.status === "OPEN",
    ).length;
    const criticalAlerts = list.filter(
      (a) =>
        a.severity === "CRITICAL" &&
        (a.status === "NEW" || a.status === "OPEN"),
    ).length;
    const warningAlerts = list.filter(
      (a) => a.severity === "HIGH" || a.severity === "MEDIUM",
    ).length;
    const acknowledgedAlerts = list.filter(
      (a) => a.status === "ACKNOWLEDGED",
    ).length;
    const resolvedToday = list.filter((a) => a.status === "RESOLVED").length;
    const totalOccurrences = list.reduce(
      (sum, a) => sum + (a.occurrenceCount || 1),
      0,
    );
    return {
  ...
  async acknowledgeAlert(
    ...
    return this.alertEngine.updateAlertStatus(
      id,
  ...
  async assignAlert(
    ...
    const performedBy = req?.user?.email || "Operator";
    return this.alertEngine.assignAlert(
      id,
      assignedUserId,
      performedBy,
      comment,
  ...
  async simulateRuleLegacy(
    ...
    @Query("threshold", new DefaultValuePipe(90), ParseIntPipe)
    threshold: number,
    @Query("timeframeHours", new DefaultValuePipe(24), ParseIntPipe)
  ...
  async createRule(@Body() body: any, @Request() req: any) {
    ...
    const ipAddress = req.ip || req.headers?.["x-forwarded-for"];
    const browser = req.headers?.["user-agent"];
    return this.ruleEngine.createRule(
      body,
      performedBy,
      undefined,
      ipAddress,
  ...
  async testRule(
    ...
    if (body.simulatedValue !== undefined && body.deviceId) {
      ...
      return (this.alertRuleEngine as any).testRule
        ? (this.alertRuleEngine as any).testRule(
            id,
            body.deviceId,
            Number(body.simulatedValue),
          )
        : {
            wouldTrigger: false,
            reason: "Test logic not implemented in service",
            durationRequired: 1,
            currentDuration: 0,
          };
    }
    ...
    return this.simulationService.testRule(
      {
        ruleId: id,
        timeframe: body.timeframe || "LAST_24H",
        from: body.from,
        to: body.to,
      },
      performedBy,
  ...
  async rollbackRule(
    ...
    const ipAddress = req.ip || req.headers?.["x-forwarded-for"];
    const browser = req.headers?.["user-agent"];
    return this.ruleEngine.rollbackToVersion(
      id,
      version,
      performedBy,
      undefined,
  ...
  async updateRule(
    ...
    const ipAddress = req.ip || req.headers?.["x-forwarded-for"];
    const browser = req.headers?.["user-agent"];
    return this.ruleEngine.updateRule(
      id,
      body,
      performedBy,
      body.reason,
  ...
  async updateStatus(
    @Param("id") id: string,
    @Body()
    body: { status: AlertStatus; performedBy?: string; comment?: string },
    @Request() req: any,
  ) {
  ...
  async bulkOperation(
    ...
    const performedBy = body.performedBy || req.user?.email || "Operator";
    return this.alertEngine.bulkOperation(
      body.alertIds,
      body.action,
      body.payload,
      performedBy,
