...
import { AlertSeverity, AlertStatus, SocketEvents, NotificationProvider } from '@nos/shared-types';
...
describe('Phase 5 - Enterprise Alert & Notification Engine', () => {
  ...
  beforeEach(async () => {
    ...
    mockAlertRepo = {
      create: jest.fn().mockImplementation((data: any) => {
        ...
        const doc = { id, occurrenceCount: 1, firstOccurred: new Date(), lastOccurred: new Date(), ...data };
      ...
      findById: jest.fn().mockImplementation((id: string) => alertsStore.get(id) || null),
      findMany: jest.fn().mockImplementation(() => [Array.from(alertsStore.values()), alertsStore.size]),
      ...
      addHistory: jest.fn().mockImplementation((data: any) => ({ id: 'hist-1', ...data, timestamp: new Date() })),
      addComment: jest.fn().mockImplementation((data: any) => ({ id: 'comm-1', ...data, createdAt: new Date() })),
      findEscalationCandidates: jest.fn().mockImplementation((mins: number) => {
        return Array.from(alertsStore.values()).filter(a => a.status === 'NEW' || a.status === 'OPEN');
      }),
      getOverviewStatistics: jest.fn().mockReturnValue({ totalAlerts: 10, openAlerts: 5, criticalAlerts: 2, warningAlerts: 3, acknowledgedAlerts: 2, resolvedToday: 3, repeatedIncidentCount: 15 }),
    };

    mockRuleRepo = {
      findMany: jest.fn().mockImplementation(() => Array.from(rulesStore.values())),
      findByName: jest.fn().mockImplementation((name: string) => {
        for (const val of rulesStore.values()) if (val.name === name) return val;
        return null;
      }),
      create: jest.fn().mockImplementation((data: any) => {
    ...
    mockMaintenanceRepo = {
      findActiveByDevice: jest.fn().mockImplementation((devId: string) => {
        const matches = Array.from(maintenanceStore.values()).filter(m => m.deviceId === devId || (!m.deviceId && !m.deviceGroupId));
        return matches;
      }),
      create: jest.fn().mockImplementation((data: any) => {
    ...
    mockNotifRepo = {
      create: jest.fn().mockImplementation((data: any) => ({ id: `notif-${Date.now()}`, ...data })),
      updateStatus: jest.fn().mockImplementation((id: string, status: string) => ({ id, status })),
  ...
  describe('SHA256 Fingerprinting & O(1) Deduplication', () => {
    it('should compute deterministic SHA256 fingerprints', () => {
      const fp1 = alertEngine.computeFingerprint('node-01', 'cpuUsage', 'rule-A');
      const fp2 = alertEngine.computeFingerprint('node-01', 'cpuUsage', 'rule-A');
      const fp3 = alertEngine.computeFingerprint('node-01', 'cpuUsage', 'rule-B');
    ...
    it('should deduplicate recurring incidents and increment occurrenceCount', async () => {
      ...
      expect(mockSocketPublisher.emitAlertEvent).toHaveBeenCalledWith(SocketEvents.ALERT_CREATED, expect.any(Object));
      ...
      expect(mockSocketPublisher.emitAlertEvent).toHaveBeenCalledWith(SocketEvents.ALERT_UPDATED, expect.any(Object));
    });
  });
  ...
  describe('Rule Simulator (Test Rule Against Historical Operations)', () => {
    it('should compute simulated triggers, suppression ratios, and estimated cooldown savings', async () => {
      ...
      expect(report.realAlertsCount).toBeLessThanOrEqual(report.wouldTriggerCount);
  ...
  describe('Notification Dispatch & DLQ Management', () => {
    it('should deliver notifications across multi-channel providers and broadcast real-time envelope', async () => {
      const res = await notificationService.dispatchNotification(NotificationProvider.SLACK, {
        alertId: 'alt-001',
      ...
      expect(mockSocketPublisher.emitAlertEvent).toHaveBeenCalledWith(SocketEvents.NOTIFICATION_SENT, expect.any(Object));
    });

    it('should format notification templates with variables correctly', () => {
      const formatted = notificationService.renderCustomTemplate('Incident {{incidentNumber}} on {{hostname}}: {{metric}} reached {{value}}', {
      ...
      expect(formatted).toBe('Incident INC-889922 on web-gw-01: RAM reached 92%');
    });
  });
  ...
  describe('Automated SLA Escalation Worker', () => {
    it('should automatically escalate severity to CRITICAL for unaddressed long-standing alerts', async () => {
      ...
      expect(mockAlertRepo.update).toHaveBeenCalledWith(oldAlert.alert?.id, { severity: AlertSeverity.CRITICAL });
      expect(mockSocketPublisher.emitAlertEvent).toHaveBeenCalledWith(SocketEvents.ALERT_ESCALATED, expect.any(Object));
    });
  });
});
