...
export class MaintenanceService {
  ...
  constructor(@Inject(IMaintenanceRepository) private readonly maintenanceRepo: IMaintenanceRepository) {}

  async createWindow(input: MaintenanceWindowCreateInput): Promise<MaintenanceWindow> {
    this.logger.log(`[Maintenance] Scheduled window "${input.title}" from ${input.startTime} to ${input.endTime} (${input.type || 'SCHEDULED'})`);
    return this.maintenanceRepo.create(input);
  }

  async isDeviceInMaintenance(deviceId: string, atTime = new Date()): Promise<{ inMaintenance: boolean; activeWindow?: MaintenanceWindow }> {
    const windows = await this.maintenanceRepo.findActiveByDevice(deviceId, atTime);
  ...
  async updateWindow(id: string, data: Partial<MaintenanceWindow>): Promise<MaintenanceWindow> {
