...
export class RuleSimulationService {
  ...
  async testRule(
    ...
    const { from, to } = this.resolveTimeframe(
      request.timeframe,
      request.from,
      request.to,
    );
    ...
    const [alerts] = await this.alertRepo.findMany({
      ruleId: rule.id,
      take: 500,
    } as any);
    ...
    const suppressed = Math.round(
      baseWouldTrigger * (0.6 + cooldownRatio * 0.2),
    );
    ...
    const actualFirings = Math.max(
      0,
      baseWouldTrigger - suppressed - deduplicated,
    );

    const estimatedNotifications = rule.silentMode ? 0 : actualFirings;
    const estimatedEmails = rule.silentMode
      ? 0
      : Math.round(actualFirings * 0.8);
  ...
  async previewRule(ruleData: {
    ...
    const deviceEstimate =
      metricCategory === "cpu" || metricCategory === "ram"
        ? Math.max(5, Math.round(totalAlerts * 0.3))
        : metricCategory === "disk"
          ? Math.max(3, Math.round(totalAlerts * 0.2))
          : metricCategory === "security"
            ? Math.max(2, Math.round(totalAlerts * 0.1))
    ...
    const estimatedDailyAlerts = Math.max(
      1,
      Math.round(deviceEstimate * volumeMultiplier * 2),
    );
    const estimatedWeeklyAlerts = estimatedDailyAlerts * 7;

    const cooldown = ruleData.cooldownSeconds ?? 300;
    const estimatedSuppression = Math.min(
      90,
      Math.round((cooldown / 3600) * 30 + 40),
    );
    const estimatedAlertVolume = Math.max(
      1,
      Math.round(estimatedDailyAlerts * (1 - estimatedSuppression / 100)),
    );
    const estimatedCorrelation = Math.round(estimatedAlertVolume * 0.1);
    const estimatedCooldownSaves = Math.round(
      (estimatedDailyAlerts * estimatedSuppression) / 100,
    );
    ...
    const complexOperator = ["CONTAINS", "NOT_CONTAINS", "MUTATED"].includes(
      operator,
    )
      ? 2
      : 0;
    const complexSchedule =
      ruleData.scheduleMode === "CRON"
        ? 2
        : ruleData.scheduleMode !== "ALWAYS"
          ? 1
          : 0;
  ...
  async dryRun(
    ...
    const startMs = Date.now();
    this.logger.log(
      `[DryRun] Starting dry run for metric="${ruleData.metric}" ${ruleData.operator} ${ruleData.threshold}`,
    );
    ...
    try {
      ...
      for (const alert of alerts.slice(0, 50)) {
        allSamples.push({
          ...
          [ruleData.metric]: alert.ruleId
            ? this.estimateMetricValue(
                ruleData.metric,
                ruleData.threshold,
                ruleData.operator,
              )
      ...
      this.logger.warn(
        `[DryRun] Could not load telemetry samples: ${err?.message}`,
      );
    }
    ...
    for (const sample of allSamples) {
      ...
      logs.push({
        ...
        wouldTrigger,
        reason: wouldTrigger
          ? `${ruleData.metric} ${ruleData.operator} ${ruleData.threshold} = TRUE`
          : suppressReason ||
            `${ruleData.metric} ${ruleData.operator} ${ruleData.threshold} = FALSE`,
        timestamp: new Date(sample.timestamp).toISOString(),
      });

      this.logger.debug(
        `[DryRun] Device ${sample.deviceId}: ${ruleData.metric}=${value} → ${wouldTrigger ? "WOULD TRIGGER" : "NO TRIGGER"}`,
      );
    }
  ...
  async replayHistoricalTelemetry(
    ...
    try {
      ...
      for (const deviceId of deviceIds) {
        ...
        const maintenance = await this.maintenanceService.isDeviceInMaintenance(
          deviceId,
          from,
        );
        ...
        const telemetry = await this.telemetryRepo.findRange({
          ...
          to,
          take: Math.min(
            this.REPLAY_MAX_SAMPLES / Math.max(deviceIds.length, 1),
            500,
          ),
        });
        ...
        for (const snapshot of telemetry.items) {
          samplesReplayed++;
          const value = this.extractMetricFromSnapshot(
            snapshot as any,
            rule.metric,
          );
          ...
          const inCooldown =
            lastFire !== undefined &&
            snapshotTime - lastFire < rule.cooldownSeconds * 1000;
  ...
  private extractMetricFromSnapshot(
  ...
  private classifyMetric(metric: string): string {
    if (["cpuUsage", "cpuTemperature", "cpuFrequency"].includes(metric))
      return "cpu";
    ...
    if (
      ...
      return "disk";
    if (
      ["security.defender", "security.firewall", "security.bitlocker"].includes(
        metric,
      )
    )
      return "security";
