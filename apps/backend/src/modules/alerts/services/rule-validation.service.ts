  ...
  async validate(
    ...
    if (percentageMetrics.includes(ruleData.metric)) {
      if (ruleData.threshold < 0 || ruleData.threshold > 100) {
        impossibleConditions.push(
          `${ruleData.metric} threshold must be 0–100 (received ${ruleData.threshold})`,
        );
        errors.push({
      ...
      if (ruleData.threshold >= 100 && ruleData.operator === ">") {
        impossibleConditions.push(
          `${ruleData.metric} > 100 will never trigger`,
        );
        errors.push({
    ...
    if (
      (ruleData.threshold ?? 0) < 0 &&
      !percentageMetrics.includes(ruleData.metric)
    ) {
      warnings.push({
    ...
    // ─── 9. Name uniqueness ────────────────────────────────
    const nameMatch = existingRules.find(
      (r) => r.name.toLowerCase() === ruleData.name.toLowerCase(),
    );
    ...
    return {
      ...
      impossibleConditions:
        impossibleConditions.length > 0 ? impossibleConditions : undefined,
    };
  }
}
