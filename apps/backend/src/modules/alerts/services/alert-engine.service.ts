...
export class AlertEngineService {
  ...
  computeFingerprint(
    deviceId: string,
    metricOrCategory: string,
    ruleId = "manual",
  ): string {
    return crypto
      .createHash("sha256")
      .update(`${deviceId}:${metricOrCategory}:${ruleId}`)
      .digest("hex");
  }
  ...
  async processIncident(data: {
    ...
    alert: Alert | null;
    status: "CREATED" | "DEDUPLICATED" | "SUPPRESSED";
  }> {
    ...
    if (maintenance.inMaintenance) {
      this.logger.log(
        `[Alert Suppressed] Device ${data.deviceId} is in active maintenance window: "${maintenance.activeWindow?.title}"`,
      );
    ...
    if (existing) {
      ...
      this.logger.log(
        `[Deduplicated] Fingerprint ${fingerprint.slice(0, 8)} occurred ${updated.occurrenceCount}x -> ${updated.incidentNumber}`,
      );
    ...
    if (metric !== "heartbeat") {
      const heartbeatFingerprint = this.computeFingerprint(
        data.deviceId,
        "heartbeat",
        "no-rule",
      );
      const parentCandidate = await this.alertRepo.findByFingerprint(
      ...
      if (parentCandidate) {
        ...
        this.logger.log(
          `[Alert Correlation] Linking consequent failure "${data.title}" under Parent Incident ${parentCandidate.incidentNumber}`,
        );
      }
    }
    ...
    this.socketPublisher.emitAlertEvent(SocketEvents.ALERT_CREATED as any, {
      alert: newAlert,
      eventType: "CREATED",
      timestamp: new Date().toISOString(),
    });
  ...
  async updateAlertStatus(
    ...
    if (newStatus === AlertStatus.RESOLVED || newStatus === AlertStatus.CLOSED)
    ...
    const socketEvent =
      newStatus === AlertStatus.ACKNOWLEDGED
        ? SocketEvents.ALERT_ACKNOWLEDGED
        : newStatus === AlertStatus.RESOLVED
          ? SocketEvents.ALERT_RESOLVED
          : SocketEvents.ALERT_UPDATED;
    this.socketPublisher.emitAlertEvent(socketEvent as any, {
      alert: updated,
  ...
  async snoozeAlert(id: string, minutes: number, performedBy = "Operator") {
    ...
    const updated = await this.alertRepo.update(id, {
      status: AlertStatus.SNOOZED as any,
      snoozedUntil,
    });
    await this.historyService.recordAction({
      alertId: id,
    ...
    this.socketPublisher.emitAlertEvent(SocketEvents.ALERT_SUPPRESSED as any, {
      alert: updated,
  ...
  async bulkOperation(
    ...
    if (action === "ACKNOWLEDGE")
      count = await this.alertRepo.bulkUpdateStatus(ids, "ACKNOWLEDGED");
    else if (action === "RESOLVE")
      count = await this.alertRepo.bulkUpdateStatus(ids, "RESOLVED");
    else if (action === "SUPPRESS")
      count = await this.alertRepo.bulkUpdateStatus(ids, "SUPPRESSED");
    else if (action === "DELETE") {
      for (const id of ids) await this.alertRepo.delete(id);
      count = ids.length;
    }
    this.logger.log(
      `[Bulk Operations] Performed ${action} on ${count} incidents by ${performedBy}`,
    );
  ...
  async getStatistics(): Promise<AlertStatisticsDto> {
    ...
    const agingBuckets: AlertAgingBuckets = {
      bucket0to5m: 0,
      bucket5to15m: 0,
      bucket15to60m: 0,
      bucket1to4h: 0,
    ...
    return {
      ...
      topRules: [
        {
          ruleId: "rule-cpu-1",
          ruleName: "CPU Critical Spike",
          count: Math.round(total * 0.45),
        },
        {
          ruleId: "rule-disk-1",
          ruleName: "Low Disk Capacity Alert",
      ...
      topDevices: [
        {
          deviceId: "dev-alpha",
          hostname: "srv-prod-db01.nos.local",
          count: Math.round(total * 0.35),
        },
        {
          deviceId: "dev-beta",
          hostname: "gw-edge-core.nos.local",
  ...
  async escalateAlert(
    ...
    await this.historyService.recordAction({
      ...
      comment:
        comment ||
        `Severity escalated from ${existing.severity} to ${targetSeverity}`,
    });
  ...
  async getCorrelatedAlerts(alertId: string): Promise<Alert[]> {
    ...
    const [all] = await this.alertRepo.findMany({
