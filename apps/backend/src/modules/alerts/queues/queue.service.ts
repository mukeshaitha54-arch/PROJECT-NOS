...
export class AlertQueueService implements OnModuleDestroy {
  ...
  private initializeQueues() {
    try {
      ...
      this.redisClient = new Redis(redisUrl, {
        ...
        retryStrategy: (times) => {
          if (times > 2) {
            this.logger.warn(`[BullMQ/Redis] Standalone Redis offline after 2 retries. Switching to fault-tolerant local async queue fallback.`);
      ...
      this.alertQueue = new Queue('AlertProcessingQueue', { connection: this.redisClient });
      this.notificationQueue = new Queue('NotificationQueue', { connection: this.redisClient });
      this.dlqQueue = new Queue('DeadLetterQueue', { connection: this.redisClient });
      this.retryQueue = new Queue('RetryQueue', { connection: this.redisClient });

      this.logger.log(`[BullMQ] Successfully registered AlertProcessingQueue, NotificationQueue, RetryQueue, and DeadLetterQueue.`);
    } catch (e: any) {
      this.isOfflineFallback = true;
      this.logger.warn(`[BullMQ] Running in local resilient fallback mode: ${e?.message}`);
    }
  }
  ...
  async enqueueAlertProcessing(data: any): Promise<void> {
    if (!this.isOfflineFallback && this.alertQueue) {
      try {
        await this.alertQueue.add('process-incident', data, { removeOnComplete: true, attempts: 3, backoff: { type: 'exponential', delay: 1000 } });
        return;
      } catch {}
    }
    // Fallback resilient execution
    this.logger.debug(`[Queue Fallback] Enqueueing alert processing for device ${data.deviceId}`);
  }

  async enqueueNotification(data: any): Promise<void> {
    if (!this.isOfflineFallback && this.notificationQueue) {
      try {
        await this.notificationQueue.add('send-notification', data, { removeOnComplete: true, attempts: 5, backoff: { type: 'exponential', delay: 2000 } });
        return;
      } catch {}
    }
    this.logger.debug(`[Queue Fallback] Enqueueing notification dispatch for incident ${data.incidentNumber}`);
  }
  ...
  async pushToDlq(data: any): Promise<void> {
    ...
    this.logger.error(`[DLQ Worker] Stored failed notification in DeadLetterQueue.`);
  }
  ...
  async getRedisHealth(): Promise<{
    ...
    if (this.isOfflineFallback || !this.redisClient) {
      return { connected: false, memoryUsageBytes: 0, latencyMs: 0, connectedClients: 0, uptime: 0 };
    }
  ...
  async purgeQueue(queueName: string): Promise<number> {
    ...
    try {
      ...
      this.logger.warn(`[Queue] Purged ${purged} failed jobs from ${queueName}`);
      return purged;
    } catch (err: any) {
  ...
  private getQueueByName(name: string): Queue | undefined {
    switch (name) {
      case 'AlertProcessingQueue': return this.alertQueue;
      case 'NotificationQueue': return this.notificationQueue;
      case 'RetryQueue': return this.retryQueue;
      case 'DeadLetterQueue': return this.dlqQueue;
  ...
  private buildOfflineQueueStats(): RuleHealthQueueInfo[] {
    return [
      { name: 'AlertProcessingQueue', waiting: 0, active: 0, completed: 0, failed: 0, delayed: 0 },
      { name: 'NotificationQueue', waiting: 0, active: 0, completed: 0, failed: 0, delayed: 0 },
      { name: 'RetryQueue', waiting: 0, active: 0, completed: 0, failed: 0, delayed: 0 },
      { name: 'DeadLetterQueue', waiting: 0, active: 0, completed: 0, failed: 0, delayed: 0 },
    ];
  }
