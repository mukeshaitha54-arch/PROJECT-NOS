...
export class EscalationWorker implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    ...
    this.logger.log(
      `[Escalation Worker] Activated 10m/20m/40m SLA automated escalation pipeline.`,
    );
  }
    ...
    try {
      ...
      for (const alert of candidates) {
        const elapsedMinutes =
          (nowMs - new Date(alert.firstOccurred).getTime()) / 60_000;

        if (elapsedMinutes >= 40 && alert.severity !== "CRITICAL") {
          const updated = await this.alertRepo.update(alert.id, {
            severity: AlertSeverity.CRITICAL as any,
          });
          ...
          this.socketPublisher.emitAlertEvent(
            SocketEvents.ALERT_ESCALATED as any,
            {
              alert: updated,
              eventType: "ESCALATED",
          ...
          elapsedMinutes >= 20 &&
          elapsedMinutes < 40 &&
          (!alert.assignedUserId || alert.assignedUserId !== "admin-pool")
        ) {
          await this.alertRepo.update(alert.id, {
            assignedUserId: "admin-pool",
          });
          await this.historyService.recordAction({
            alertId: alert.id,
      ...
      if (escalatedCount > 0) {
        this.logger.log(
          `[Escalation Worker] Processed ${escalatedCount} automated SLA escalations across active alerts.`,
        );
      }
