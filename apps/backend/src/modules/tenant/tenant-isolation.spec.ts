...
describe('Phase 6 - Multi-Tenant SaaS Transformation & Cross-Tenant Safety Integration Verification', () => {
  ...
  beforeEach(async () => {
    ...
    const mockOrgRepo: Partial<IOrganizationRepository> = {
      ...
      findById: jest.fn().mockImplementation(async (id: string) => orgStore.get(id) || null),
      ...
      updateStatus: jest.fn().mockImplementation(async (id: string, status: any) => {
      ...
      updateSettings: jest.fn().mockImplementation(async (id: string, settings: any) => {
        const org = orgStore.get(id);
      ...
      getQuota: jest.fn().mockResolvedValue({ maxDevices: 100, maxUsers: 50, maxApiKeys: 20, maxStorageMb: 5000, retentionDays: 30, maxDailyTelemetry: 100000, maxDailyAlerts: 5000 }),
      updateQuota: jest.fn().mockImplementation(async (orgId, q) => ({ maxDevices: 100, maxUsers: 50, maxApiKeys: 20, maxStorageMb: 5000, retentionDays: 30, maxDailyTelemetry: 100000, maxDailyAlerts: 5000, ...q })),
      listAll: jest.fn().mockResolvedValue({ items: [], total: 0, page: 1, totalPages: 0 }),
    };

    const mockTeamRepo: Partial<ITeamRepository> = {
      addMember: jest.fn().mockImplementation(async (orgId, userId, role, teamIds, deptIds, customRoleId) => {
        const key = `${orgId}:${userId}`;
        const doc = { id: `mem-${Math.random()}`, organizationId: orgId, userId, role, customRoleId, createdAt: new Date() };
        teamStore.set(key, doc);
        return doc;
      }),
      findMember: jest.fn().mockImplementation(async (orgId, userId) => teamStore.get(`${orgId}:${userId}`) || null),
      listMembers: jest.fn().mockImplementation(async (orgId) => {
        const items = Array.from(teamStore.values()).filter(m => m.organizationId === orgId);
      ...
      createInvitation: jest.fn().mockImplementation(async (orgId, email, role, inviterId) => {
        return { id: `inv-${Math.random()}`, organizationId: orgId, email, role, inviterId, token: 'secret-token', expiresAt: new Date(Date.now() + 86400000), status: 'PENDING' };
      }),
      listInvitations: jest.fn().mockResolvedValue([]),
      revokeInvitation: jest.fn().mockResolvedValue(undefined),
      updateMemberRole: jest.fn().mockImplementation(async (orgId, userId, role, customRoleId) => {
        const doc = teamStore.get(`${orgId}:${userId}`) || { id: `mem-${Math.random()}`, organizationId: orgId, userId, role, customRoleId };
        doc.role = role;
    ...
    const mockSessionRepo: Partial<IUserSessionRepository> = {
      listActiveSessions: jest.fn().mockResolvedValue([
        { id: 'sess-1', organizationId: 'org-enterprise-alpha', userId: 'user-owner-a', ipAddress: '10.0.1.50', browser: 'Chrome', active: true, createdAt: new Date(), lastSeenAt: new Date() }
      ]),
    ...
    const mockAuditRepo: Partial<IAuditLogRepository> = {
      record: jest.fn().mockImplementation(async (entry) => {
        const doc = { id: `audit-${Date.now()}-${Math.random()}`, timestamp: new Date(), ...entry };
        auditStore.push(doc);
        return doc;
      }),
      search: jest.fn().mockImplementation(async (filter) => {
        const matches = auditStore.filter(a => a.organizationId === filter.organizationId);
        return { items: matches, total: matches.length, page: 1, totalPages: 1 };
      }),
    };

    const mockApiKeyRepo: Partial<IApiKeyRepository> = {
      listByOrganization: jest.fn().mockResolvedValue({ items: [], total: 2, page: 1, totalPages: 1 }),
    };
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: IDeviceGovernanceRepositoryToken, useValue: mockGovernanceRepo },
  ...
  describe('1. Cross-Tenant Repository & Service Isolation Enforcement', () => {
    it('should strictly isolate organization member queries between tenants A and B', async () => {
      await userService.inviteUser('org-enterprise-alpha', 'devops@alpha-corp.com', UserRole.ADMIN, orgContextA);
      await userService.inviteUser('org-enterprise-beta', 'secops@beta-corp.com', UserRole.MANAGER, orgContextB);

      // Simulate members added in team repository
      await userService.updateMemberRole('org-enterprise-alpha', 'user-alpha-1', UserRole.ADMIN, orgContextA);
      await userService.updateMemberRole('org-enterprise-beta', 'user-beta-1', UserRole.MANAGER, orgContextB);

      // Verify Org A list only contains Org A users
      const listA = await userService.listMembers('org-enterprise-alpha');
      expect(listA.items.every(m => m.organizationId === 'org-enterprise-alpha')).toBe(true);
      expect(listA.items.find(m => m.userId === 'user-beta-1')).toBeUndefined();

      // Verify Org B list only contains Org B users
      const listB = await userService.listMembers('org-enterprise-beta');
      expect(listB.items.every(m => m.organizationId === 'org-enterprise-beta')).toBe(true);
      expect(listB.items.find(m => m.userId === 'user-alpha-1')).toBeUndefined();
    });
    ...
    it('should prohibit audit history leakage across tenant boundaries', async () => {
      ...
      await auditService.logEvent(orgContextA, AuditActionType.DEVICE_REGISTRATION, 'DEVICE', 'router-core-01', 'Claimed core enterprise router');
      await auditService.logEvent(orgContextB, AuditActionType.API_KEY_CREATE, 'API_KEY', 'key-beta-prod', 'Generated production telemetry API key');

      const auditA = await auditService.search({ organizationId: 'org-enterprise-alpha' });
      const auditB = await auditService.search({ organizationId: 'org-enterprise-beta' });

      expect(auditA.items.every(log => log.organizationId === 'org-enterprise-alpha')).toBe(true);
      expect(auditA.items.find(log => log.resourceId === 'key-beta-prod')).toBeUndefined();

      expect(auditB.items.every(log => log.organizationId === 'org-enterprise-beta')).toBe(true);
      expect(auditB.items.find(log => log.resourceId === 'router-core-01')).toBeUndefined();
    });
  });
  ...
  describe('2. Dynamic RBAC Evaluation & Privilege Containment', () => {
    it('should grant exhaustive privileges to OWNER and SUPER_ADMIN roles', async () => {
      const isOwnerAllowed = await rbacService.hasPermission('org-enterprise-alpha', UserRole.OWNER, PermissionFlag.ROLE_BUILDER_MANAGE);
      const isSuperAllowed = await rbacService.hasPermission('org-enterprise-alpha', UserRole.SUPER_ADMIN, PermissionFlag.AUDIT_READ);
    ...
    it('should enforce strict privilege separation for OPERATOR role (No RBAC builder or administrative tenant control)', async () => {
      const hasDevicePerm = await rbacService.hasPermission('org-enterprise-alpha', UserRole.OPERATOR, PermissionFlag.DEVICE_MANAGEMENT);
      const hasRolePerm = await rbacService.hasPermission('org-enterprise-alpha', UserRole.OPERATOR, PermissionFlag.ROLE_BUILDER_MANAGE);
  ...
  describe('3. WebSocket Room Isolation & Zero Cross-Tenant Emission', () => {
    it('should route tenant notification payloads strictly to dynamic room org_{organizationId}', async () => {
      ...
      mockSocketPublisher.emitTenantEvent('org-enterprise-alpha', 'alert:created', { alertId: 'alert-sec-01', severity: 'HIGH' });
      ...
      expect(mockSocketPublisher.emitTenantEvent).not.toHaveBeenCalledWith('org-enterprise-beta', expect.any(String), expect.any(Object));
  ...
  describe('5. Enterprise Quota Engine & Consumption Enforcement', () => {
    it('should permit resource provisioning when quota limits are within normal operational bounds', async () => {
      ...
      await expect(quotaService.checkQuotaConsumption('org-enterprise-alpha', 'DEVICES')).resolves.not.toThrow();
    });
    ...
    it('should block provisioning and emit ErrorCode.QUOTA_EXCEEDED when tenant hits resource ceiling', async () => {
      ...
      await expect(quotaService.checkQuotaConsumption('org-enterprise-alpha', 'DEVICES')).rejects.toThrow(BadRequestException);
    });
  });
});
