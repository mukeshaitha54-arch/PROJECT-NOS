...
export class OrganizationService {
  constructor(
    ...
    @Inject(IAuditLogRepositoryToken) private readonly auditRepository: IAuditLogRepository,
  ...
  async getById(id: string): Promise<OrganizationDto> {
    ...
    if (!org) {
      throw new NotFoundException({ code: 'RESOURCE_NOT_FOUND', message: `Organization [${id}] not found.` });
    }
  ...
  async create(
    data: { name: string; slug: string; settings?: Partial<OrganizationSettingsDto>; quota?: Partial<OrganizationQuotaDto> },
    context: TenantContext,
  ): Promise<OrganizationDto> {
    const existing = await this.orgRepository.findBySlug(data.slug);
    if (existing) {
      throw new BadRequestException({ code: ErrorCode.VALIDATION_ERROR, message: `Organization slug [${data.slug}] is already taken.` });
    }
  ...
  async updateStatus(id: string, status: OrganizationStatus, context: TenantContext, reason?: string): Promise<OrganizationDto> {
  ...
  async updateSettings(id: string, settings: OrganizationSettingsDto, context: TenantContext): Promise<OrganizationDto> {
    await this.getById(id);
  ...
  async archive(id: string, context: TenantContext, reason: string): Promise<void> {
    await this.updateStatus(id, OrganizationStatus.ARCHIVED, context, reason);
  }

  async softDelete(id: string, context: TenantContext, reason: string): Promise<void> {
    await this.getById(id);
  ...
  async restore(id: string, context: TenantContext, reason: string): Promise<OrganizationDto> {
  ...
  async listAll(params?: { search?: string; status?: string; page?: number; limit?: number }) {
