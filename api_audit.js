...
frontendFiles.forEach((file) => {
  ...
  const regex =
    /apiClient\.(get|post|put|patch|delete)\b[^(]*\(\s*(['"`])(.*?)\2/g;
  let match;
...
frontendEndpoints.forEach((fe) => {
  ...
  const match = backendEndpoints.find(
    (be) => be.method === fe.method && normalizeRoute(be.route) === feNorm,
  );

  if (match) {
    table.push(
      `| ${fe.file} | \`${fe.method} ${fe.route}\` | \`${fe.method} ${match.route}\` | \`${fe.method} ${match.route}\` | ✅ Matched |`,
    );
  } else {
    table.push(
      `| ${fe.file} | \`${fe.method} ${fe.route}\` | \`${fe.method} ${fe.route}\` | ❌ MISSING/404 | ❌ 404 |`,
    );
  }
});
